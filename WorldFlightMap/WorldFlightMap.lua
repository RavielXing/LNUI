...
-- 本地化常用全局函数与 API 表，减少全局表查找开销
local InCombatLockdown = InCombatLockdown
local IsInInstance = IsInInstance
local GetInstanceInfo = GetInstanceInfo
local GetTaxiMapID = GetTaxiMapID
local GetNumRoutes = GetNumRoutes
local TaxiGetNodeSlot = TaxiGetNodeSlot
local CloseTaxiMap = CloseTaxiMap
local pairs, wipe, tonumber = pairs, wipe, tonumber
local C_Map = C_Map
local C_TaxiMap = C_TaxiMap
...
-- 12.1 内存优化：限制缓存规模，超过上限时整体清空（地图坐标数据随时可重新计算）
local MAP_CACHE_LIMIT = 40
local function PruneMapCache()
	local count = 0
	for _ in pairs(MapSizeCache) do
		count = count + 1
	end
	if count > MAP_CACHE_LIMIT then
		wipe(MapSizeCache)
	end
end
...
local function GetParentZone(uiMapID)
	if uiMapID then
		local zone = MapUtil.GetMapParentInfo(uiMapID, Enum.UIMapType.Zone, uiMapID ~= 2346)
		if zone then
			return zone.mapID
		end
...
local function GetMapSize(uiMapID, noLoop)
	uiMapID = uiMapID or WorldMapFrame:GetMapID()
	...
	local instanceID2, bottomright = C_Map.GetWorldPosFromMapPos(uiMapID, {x = 1, y = 1})
	if not (instanceID and instanceID2) then return end
...
-- 12.1 内存优化：缓存当前地图的 taxi 节点查询结果，
-- 避免 AddFlightNode 每绘制一个飞行点都新建一张完整节点表
local TaxiNodeCache = {} -- [mapID] = taxiNodes

local function GetCachedTaxiNodes(mapID)
	local nodes = TaxiNodeCache[mapID]
	if not nodes then
		nodes = C_TaxiMap.GetTaxiNodesForMap(mapID)
		TaxiNodeCache[mapID] = nodes
	end
	return nodes
end
...
-- Arrow functions
-- 12.1 内存优化：弱键表，图钉回收后箭头框架可被 GC 释放
local PinArrows = setmetatable({}, {__mode = 'k'}) -- [pin] = arrow frame
...
local function GetArrow(pin)
	...
	local existing = PinArrows[pin]
	if existing and existing.pin == pin then
		return existing
	end
			...
			if IsInInstance() then
				...
				if instanceID == 2481 or instanceID == 2657 or instanceID == 2769 then
					if not C_AddOns.IsAddOnLoaded('Blizzard_FlightMap') then
						C_AddOns.UIParentLoadAddOn('Blizzard_FlightMap')
...
-- 12.1 内存优化：常数 e 直接内联，避免每次调用 math.exp
...
function WorldFlightMapProvider:AddFlightNode(taxiNodeData)
	if self.taxiMap and self.worldMap and self.worldMap.left then
		-- 12.1 fix: relax continent filter for special zones
		local skipContinentCheck = false
		if self.taxiMap.mapID == self.worldMap.mapID then
			skipContinentCheck = true
		end
		-- 12.1 fix: 阿塔乌特克地窟(16365)加入continent跳过白名单
		local worldMapID = self.worldMap.mapID
		if worldMapID == 2600 or worldMapID == 2601 or worldMapID == 16365 then
			skipContinentCheck = true
		end
		if skipContinentCheck or (self.worldMap.continent == self.playerContinent) then
			...
			local drawPin = false
			-- 12.1 内存优化：使用缓存的节点查询结果，每张地图只查一次
			local taxiNodes = GetCachedTaxiNodes(worldMapID)
			for _, landmark in pairs(taxiNodes) do
				-- 12.1 fix: tonumber确保nodeID类型匹配
				if tonumber(landmark.nodeID) == tonumber(taxiNodeData.nodeID) then
					-- 12.1 fix: 使用SetXY安全赋值
					if taxiNodeData.position.SetXY then
						taxiNodeData.position:SetXY(landmark.position.x, landmark.position.y)
					else
			...
			-- 12.1 fix: 恢复坐标转换回退逻辑，防止飞行点坐标错误导致无法点击
			if not drawPin then
				if taxiNodeData.position.SetXY then
					taxiNodeData.position:SetXY(mapTaxiX, mapTaxiY)
				else
...
function WorldFlightMapProvider:RemoveAllData()
	...
	-- 12.1 内存优化：清理 _G 中残留的 TaxiButton 引用，
	-- 防止每次与飞行管理员对话都向全局表添加新条目
	if self.slotIndexToPin then
		for slotIndex in pairs(self.slotIndexToPin) do
			_G['TaxiButton' .. slotIndex] = nil
		end
		wipe(self.slotIndexToPin)
	end

	-- 12.1 内存优化：清空 taxi 节点查询缓存
	wipe(TaxiNodeCache)
end

function WorldFlightMapProvider:HighlightRouteToPin(pin)
	if self.playerContinent == 905 then return end -- don't draw lines on argus maps
...
function WorldFlightMapProvider:RemoveRouteToPin(pin)
	...
	for _, p in next, self.slotIndexToPin do
		-- update visibility
		p:SetShown(p.taxiNodeData.state ~= Enum.FlightPathState.Unreachable)
	end
end
