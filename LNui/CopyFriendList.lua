...
CoreUIEnableTooltip(btn, LOCALE_zhCN and "点击复制同帐号好友", "点击鼠标按钮会弹出已记录好友的同帐号角色，选中可将其好友复制到本角色中。\n \n撤销上次复制功能可用来删除刚刚复制的好友。" or "點擊復製同帳號好友", "點擊鼠標按鈕會彈出已記錄好友的同帳號角色，選中可將其好友復製到本角色中。\n \n撤銷上次復製功能可用來刪除剛剛復製的好友。")
...
local function copyfriends(_, name)
    ...
    StaticPopupDialogs.U1_COPY_FRIEND_LIST_CONFIRM.text = LOCALE_zhCN and '将要添加 <'..
        name .. '> 下 ' .. fut_num .. ' 位好友\n完成后你将会有 '..
        (cur_num + fut_num) .. ' 位好友' or '將要添加 <'..
        name .. '> 下 ' .. fut_num .. ' 位好友\n完成後你將會有 '..
        (cur_num + fut_num) .. ' 位好友'
...
menu.initialize = function()
    ...
    info.text = LOCALE_zhCN and '好友复制' or '好友復製'
    ...
    info.isNotRadio = true
    info.notCheckable = true
    info.text = LOCALE_zhCN and '撤销上次复制' or '撤銷上次復製'
    info.func = restore
    UIDropDownMenu_AddButton(info)
end
