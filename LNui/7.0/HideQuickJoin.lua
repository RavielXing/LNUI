...
function U1Toggle_QuickJoinToasts(allow, loading)
    if allow then
        ...
        if not loading then U1Message(LOCALE_zhCN and "已允许快速加入提示" or "已允許快速加入提示", .5, 1, .5) end
    else
        btn:UnregisterEvent("SOCIAL_QUEUE_UPDATE")
        btn.Toast:SetParent(hider)
        btn.Toast2:SetParent(hider)
        U1Message(LOCALE_zhCN and "已屏蔽快速加入提示，可在'老农小功能集'中设置" or "已屏蔽快速加入提示，可在'老农小功能集'中設置", 1, .5, .5)
    end
end
