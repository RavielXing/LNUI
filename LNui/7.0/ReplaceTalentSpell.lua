...
CoreOnEvent("PLAYER_SPECIALIZATION_CHANGED", function(event, unit)
    ...
    for old_id, new_id in pairs(old_2_new) do
        ...
        if selected then
            ...
            U1Message(LOCALE_zhCN and "已自动将动作条上的" or "已自動將動作條上的"..GetSpellLink(old_spell_id)..LOCALE_zhCN and "替换为" or "替換為"..GetSpellLink(new_spell_id))
            for i = 1, 120 do
                local type, spell_id = GetActionInfo(i)
                if type == "spell" then
...
function U1ScanSpells()
    ...
    local getlink = function(spellId, should)
        ...
        line = line:gsub(LOCALE_zhCN and "#法术编号[^#]+$" or "#法術編號[^#]+$", "")
        tinsert(result, line)
    ...
    for i=2,GetNumSpellTabs() do
        ...
        all = all..LOCALE_zhCN and " 天赋-" or " 天賦-"
        for row = 1, MAX_TALENT_TIERS do
            curr_type = spec .. name .. LOCALE_zhCN and "-天赋" or "-天賦"..row
            for col = 1, 3 do
