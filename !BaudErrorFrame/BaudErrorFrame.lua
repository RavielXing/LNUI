...
--  fuck BugGrabber embeded in the fucking idots Decursive
    ...
    LibBugGrabber.RegisterCallback = Noop;
    LibBugGrabber.UnregisterCallback = Noop;
    LibBugGrabber.setupCallbacks = Noop;
    LibBugGrabber.StoreError = Noop;
    function LibBugGrabber:GetChatLink()
        return "";
    end
    LibBugGrabber.GetErrorByPlayerAndID = Noop;
    LibBugGrabber.GetErrorByID = Noop;
    function LibBugGrabber:GetErrorID(errorObject)
        return tostring(errorObject):sub(8);
    end
    LibBugGrabber.Reset = Noop;
    function LibBugGrabber:GetDB()
        return EmptyTable;
    end
    function LibBugGrabber:GetSessionId()
        return -1;
    end
    function LibBugGrabber:IsPaused()
        return false;
    end
    LibBugGrabber.HandleBugLink = Noop;


    _G.BugGrabber = LibBugGrabber
--
...
local CheckButtons = {
    {Text="重登录时不清除错误信息", SavedVar="SaveToVariable", Default=false},
...
local function BaudErrorFrameCreateLocalButton()
...
local function BaudErrorFrameEditBoxUpdateLocal(self)
...
local function BaudErrorFrameAdd(Error, Retrace)
    ...
    -- 12.0适配：debugstack/debuglocals在受限环境中可能被限制，使用pcall保护
    local error = {Error=Error, Count=1, Stack="", locals={} }
...
local function BaudErrorFrameHandler(Error)
    BaudErrorFrameAdd(Error,4);
end

local function BaudErrorFrameMinimapButton_OnClick(self, button)
...
local function BaudErrorFrameMinimapButton_OnEnter(self)
...
local function BaudErrorFrameMinimapButton_OnLeave(self)
    if GameTooltip:IsOwned(self) then
        GameTooltip:Hide();
    end
end
local function BaudErrorFrameMinimapButton_Create()
...
EventFuncs = {
    ADDON_LOADED = function(addon)
        if addon == AddonName then
            ...
            BaudErrorFrameSoundEditBox:SetText(Config.Sound);
            BaudErrorFrameSoundEditBox:SetCursorPosition(0);
            BaudErrorFrameSoundEditBox:SetScript("OnChar", function(self)
...
function BaudErrorFrame_OnHide(self)
    PlaySound(SOUNDKIT and SOUNDKIT.GS_TITLE_OPTION_EXIT or "gsTitleOptionExit");
end
function BaudErrorFrameEntry_OnClick(self)
    SelectedError = self:GetID();
    BaudErrorFrameScrollBar_Update();
...
function BaudErrorFrameOptions_OnLoad(self)
    ...
    Text:SetText("Version "..GetAddOnMetadata(AddonName,"Version"));
    ...
    for Key, Value in ipairs(CheckButtons)do
        Button = CreateFrame("CheckButton", "BaudErrorFrameCheck"..Key, self, "OptionsCheckButtonTemplateBaud");
        Value.Button = Button;
