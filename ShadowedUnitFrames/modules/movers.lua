...
-- Per-unit test mode: shows a full placeholder frame for a specific unit type
...
local function createTestConfigEnv()
	...
	-- Build an env that delegates to configEnv fakes only for test mode unit types,
	-- and to _G for everything else. This prevents polluting live frames.
	local cache = {}
		...
		__index = function(tbl, key)
			...
			if( type(fakeVal) == "function" and type(realVal) == "function" ) then
				cache[key] = function(firstArg, ...)
					-- Most Unit* APIs take a unit token as first arg
...
function Movers:EnableTestMode(unitType)
	...
	-- For header-based units (party, raid, etc.), force-create and show the header
