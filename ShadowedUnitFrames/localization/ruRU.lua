...
L["0 removes the cap (uses the frame's rows and icons per row)."] = "0 снимает ограничение (используются ряды и значки в ряду рамки)."
L["A filter with this name already exists."] = "Фильтр с таким именем уже существует."
L["Accepts a spell ID, a spell link (shift-click a spell) or an exact spell name. You can also drag a spell from the spellbook onto this field. Spell names only resolve for spells your character currently knows; use the spell ID for anything else."] = "Принимает ID заклинания, ссылку на заклинание (Shift-клик) или точное название. Можно также перетащить заклинание из книги заклинаний. Названия распознаются только для известных персонажу заклинаний; для остальных используйте ID."
L["Add section"] = "Добавить секцию"
L["Added %s (#%d)."] = "%s (#%d) добавлено."
L["Adds an Essence bar for Evokers."] = "Добавляет полосу сущности для пробудителей."
L["Adds another filter to this frame, shown after the previous sections. Overlaps are removed automatically."] = "Добавляет рамке ещё один фильтр, отображаемый после предыдущих секций. Пересечения убираются автоматически."
L["Adds the spell ID to aura tooltips, handy for filling the spell fields of custom filters and aura indicators."] = "Добавляет ID заклинания в подсказки аур — удобно для заполнения полей заклинаний в пользовательских фильтрах и индикаторах аур."
L["All tooltips"] = "Все подсказки"
L["All tooltips except auras"] = "Все подсказки, кроме аур"
L["Anchor to buffs %d"] = "Прикрепить к баффам %d"
L["Anchor to debuffs %d"] = "Прикрепить к дебаффам %d"
L["Anchors this buff frame to the corresponding debuff frame, positioning it after the last visible debuff."] = "Прикрепляет эту рамку баффов к соответствующей рамке дебаффов, после последнего видимого дебаффа."
L["Anchors this debuff frame to the corresponding buff frame, positioning it after the last visible buff."] = "Прикрепляет эту рамку дебаффов к соответствующей рамке баффов, после последнего видимого баффа."
L["Any dispellable"] = "Все рассеиваемые ауры"
L["Application order"] = "Порядок наложения"
L["Assign a filter to each unit frame per zone type. Filters are created and edited in the Custom filters tab."] = "Назначьте фильтр каждой рамке по типу зоны. Фильтры создаются и редактируются во вкладке «Пользовательские фильтры»."
L["Aura borders keep their neutral color instead of being tinted by dispel type. Has no effect with the Blizzard border style."] = "Рамки аур сохраняют нейтральный цвет вместо окраски по типу рассеивания. Не действует со стилем рамки Blizzard."
L["Big defensives first"] = "Сначала мощные защитные"
L["Bleed"] = "Кровотечение"
L["Blizzard only applies spell filters to buffs on friendly units and to debuffs on hostile units, plus any spell it flags as never secret."] = "Blizzard применяет фильтры заклинаний только к баффам союзных и дебаффам враждебных юнитов, плюс заклинания, помеченные как никогда не секретные."
L["Border coloring of stealable buffs. Replaces the dispel type color on those auras."] = "Цвет рамки похищаемых баффов. Заменяет цвет типа рассеивания на этих аурах."
L["Buffs flagged by Blizzard to show on raid frames in combat, HoTs mostly."] = "Баффы, помеченные Blizzard для отображения на рейдовых рамках в бою, в основном HoT."
L["Cancelable auras"] = "Отменяемые ауры"
L["Cancelable Auras"] = "Отменяемые ауры"
L["Class auras (group, raid)"] = "Классовые ауры (группа, рейд)"
L["Color and opacity of the overlay pulsing on your own auras inside their pandemic window. Enabled in the general aura settings."] = "Цвет и прозрачность оверлея, пульсирующего на ваших аурах в окне пандемии. Включается в общих настройках аур."
L["Colors used for dispel type coloring: aura borders, health bar tinting and dispel highlighting."] = "Цвета для окраски по типу рассеивания: рамки аур, тонировка полосы здоровья и подсветка рамки."
L["Contextual auras (group, raid)"] = "Контекстные ауры (группа, рейд)"
L["Cooldown swipe opacity"] = "Прозрачность затемнения перезарядки"
L["Create filter"] = "Создать фильтр"
L["Curated list of class and healer spells. Combat display works for buffs on friendly units; the missing option requires a Blizzard-whitelisted spell."] = "Подобранный список заклинаний классов и лекарей. Отображение в бою работает для баффов на союзных юнитах; опции «если отсутствует» нужно заклинание из белого списка Blizzard."
L["Curse"] = "Проклятие"
L["Custom"] = "Пользовательский"
L["Custom filters"] = "Пользовательские фильтры"
L["Custom filters let you build aura filters from a spell's name or spell ID. Pick a mode when creating one: \"Show only matching spells\" or \"Hide matching spells\"."] = "Пользовательские фильтры строят фильтры аур по названию или ID заклинания. При создании выберите режим: \"Показывать только указанные заклинания\" или \"Скрывать указанные заклинания\"."
L["Default"] = "По умолчанию"
L["Disable dispel type borders"] = "Отключить рамки по типу рассеивания"
L["Disable right-click cancel"] = "Отключить отмену правым кликом"
L["Disable stack counts"] = "Отключить счётчик стаков"
L["Disable stealable coloring"] = "Отключить окраску похищаемых баффов"
L["Disease"] = "Болезнь"
L["Dispellable Auras"] = "Рассеиваемые ауры"
L["Dispellable by group"] = "Рассеивается группой"
L["Dispellable by my group"] = "Рассеивается моей группой"
L["Enable"] = "Включить"
L["Enlarge my auras"] = "Увеличивать мои ауры"
L["Enlarged scale"] = "Масштаб увеличения"
L["Enrage"] = "Исступление"
L["Essence"] = "Сущность"
L["Expiration"] = "Истечение"
L["Expiration (yours first)"] = "Истечение (ваши первыми)"
L["Filter"] = "Фильтр"
L["Frame changes are deferred while in combat and will apply afterwards."] = "Изменения рамок откладываются в бою и применяются после него."
L["Gap in pixels between aura icons on the same row, negative values overlap them."] = "Расстояние в пикселях между значками аур в одном ряду, отрицательные значения накладывают их друг на друга."
L["Gap in pixels between aura rows, negative values overlap them."] = "Расстояние в пикселях между рядами аур, отрицательные значения накладывают их друг на друга."
L["Group"] = "Группа"
L["Group this aura will be listed under. Both a spell and a group are required to add an aura."] = "Группа, в которой будет числиться аура. Для добавления нужны и заклинание, и группа."
L["Hide matching spells"] = "Скрывать указанные заклинания"
L["Hides the stack count text on all Shadowed Unit Frame auras."] = "Скрывает текст стаков на всех аурах Shadowed Unit Frames."
L["HoTs & raid frames buffs"] = "HoT и баффы рейдовых рамок"
L["Horizontal spacing"] = "Горизонтальный интервал"
L["How auras in this frame are ordered. Sorting is per section."] = "Порядок аур в этой рамке. Сортировка идёт по секциям."
L["If you don't, you will be unable to use any combo point features (Chi, Holy Power, Combo Points, Aura Points, etc) until you do so."] = "Иначе функции очков серии (ци, энергия света, очки серии, очки аур и т.д.) будут недоступны, пока вы этого не сделаете."
L["Important only"] = "Только важные"
L["In combat, Blizzard only applies spell filters to buffs on friendly units and debuffs on hostile units, plus spells it flags as never secret."] = "В бою Blizzard применяет фильтры заклинаний только к баффам союзных и дебаффам враждебных юнитов, плюс заклинания, помеченные как никогда не секретные."
L["Link from"] = "Связать из"
L["Link to"] = "Связать с"
L["Magic"] = "Магия"
...
L["Move up"] = "Вверх"
L["My Auras"] = "Мои ауры"
L["My HoTs & raid frames buffs"] = "Мои HoT и баффы рейдовых рамок"
L["Name (yours first)"] = "Имя (ваши первыми)"
L["Neighborhood"] = "Квартал"
L["Off:%s"] = "Выкл:%s"
L["Only shows auras that have a dispel type, whether or not anyone in your group can dispel them."] = "Показывает только ауры с типом рассеивания, независимо от того, может ли кто-то в группе их рассеять."
L["Only shows buffs that someone in your group can purge, steal or dispel."] = "Показывает только баффы, которые кто-то в вашей группе может снять, украсть или рассеять."
L["Only shows debuffs that someone in your group can dispel (you, when solo)."] = "Показывает только дебаффы, которые кто-то в вашей группе может рассеять (в одиночку — вы)."
L["Only shows debuffs that you can dispel."] = "Показывает только дебаффы, которые можете рассеять вы."
L["Opacity of the dark overlay showing the remaining time on auras."] = "Прозрачность тёмного слоя, показывающего оставшееся время аур."
L["Pandemic overlay"] = "Оверлей пандемии"
L["Poison"] = "Яд"
L["Private auras from bosses and encounters displayed using Blizzard's secure API."] = "Приватные ауры боссов и сражений, отображаемые через защищённый API Blizzard."
L["Profile changes will be applied after combat."] = "Изменения профиля применятся после боя."
L["Pulses an overlay on your own auras inside their pandemic window, where refreshing them carries the remaining time over. Color and opacity are set in the Colors tab."] = "Показывает пульсирующий оверлей на ваших аурах, пока они в окне пандемии — обновление в этот момент переносит оставшееся время. Цвет и прозрачность настраиваются во вкладке «Цвета»."
L["Right-clicking your own buffs cancels them; enable this to turn that off."] = "Правый клик по вашим баффам отменяет их; включите эту опцию, чтобы отключить такое поведение."
L["Scale multiplier applied to player auras when enlargement is enabled."] = "Множитель масштаба для аур игрока при включённом увеличении."
L["Scale up auras that were cast by the player to make them more visible."] = "Увеличивает ауры, применённые игроком, чтобы они были заметнее."
L["Section"] = "Секция"
L["Sections add extra filters to this frame, shown after the main filter. Auras already shown by a previous section are removed automatically. Each section has its own icon size, sorting and icon cap."] = "Секции добавляют рамке дополнительные фильтры, отображаемые после основного. Ауры, уже показанные предыдущей секцией, убираются автоматически. У каждой секции свои размер значков, сортировка и лимит значков."
L["Set Filter Zones"] = "Задать зоны фильтра"
L["Shadow"] = "Тень"
L["Shadow color"] = "Цвет тени"
L["Shadow X"] = "Тень X"
L["Shadow Y"] = "Тень Y"
L["Show cooldown"] = "Показывать перезарядку"
L["Show cooldown numbers"] = "Показывать цифры перезарядки"
L["Show only matching spells"] = "Показывать только указанные заклинания"
L["Show spell IDs in aura tooltips"] = "Показывать ID заклинаний в подсказках аур"
L["Shows placeholder auras on this unit's frames to preview positioning."] = "Показывает ауры-образцы на рамках юнита для предпросмотра расположения."
L["Smart Friendly/Hostile Filter"] = "Умный фильтр свой/чужой"
L["Solid color to use in the indicator, only used if you do not have use aura icon enabled."] = "Сплошной цвет индикатора, используется только без значка ауры."
L["Some aura changes are deferred while combat restrictions are active (combat, dungeons, raids)."] = "Некоторые изменения аур откладываются, пока действуют боевые ограничения (бой, подземелья, рейды)."
L["Spell"] = "Заклинание"
L["Spell you want to link to a primary aura, the casing must be exact."] = "Заклинание для привязки к основной ауре; регистр должен совпадать точно."
L["Stacks"] = "Стаки"
L["Stacks font"] = "Шрифт стаков"
L["Stealable"] = "Похищаемые"
L["Stealable buffs are tinted by dispel type like the others."] = "Похищаемые баффы окрашиваются по типу рассеивания, как и остальные."
L["Summon Pending"] = "Ожидание призыва"
L["Test mode"] = "Тестовый режим"
L["Test mode cannot be toggled while in combat."] = "Тестовый режим нельзя переключить в бою."
L["The check boxes below will allow you to enable or disable units.|n|n|cffff2020Warning!|r Target of Target units have a higher performance cost compared to other units. If you have performance issues, please disable those units or reduce the features enabled for those units."] = "Флажки ниже включают и отключают юнитов.|n|n|cffff2020Внимание!|r Юниты «цель цели» затратнее прочих. При проблемах с производительностью отключите их или сократите их функции."
L["Timer"] = "Таймер"
L["Timer font"] = "Шрифт таймера"
L["Unchecking this will completely disable aura indicators for %s."] = "Снятие флажка полностью отключает индикаторы аур для %s."
L["Unit changes will be applied after combat."] = "Изменения юнитов применятся после боя."
L["Unit frame debuff order"] = "Порядок дебаффов рамок юнитов"
L["Unit frames cannot be unlocked while in combat."] = "Рамки юнитов нельзя разблокировать в бою."
L["Units that should have the aura groups settings changed below."] = "Юниты, чьи настройки групп аур меняются ниже."
L["Units to change"] = "Юниты для изменения"
L["Unknown spell. Enter a spell ID, a spell link or an exact spell name."] = "Неизвестное заклинание. Введите ID, ссылку или точное название заклинания."
L["Use Boss Mod Cast Name overrides"] = "Использовать названия заклинаний от босс-модов"
L["Use spell name overrides provided by boss mods (BigWigs) on the cast bars."] = "Использует названия заклинаний от босс-модов (BigWigs) на полосах применения."
L["Uses the icon of the totem being shown instead of a status bar."] = "Использует значок показываемого тотема вместо полосы состояния."
L["Vertical spacing"] = "Вертикальный интервал"
L["WARNING! ShadowedUF_Indicators has been deprecated as v4 and is now built in. Please delete ShadowedUF_Indicators, your configuration will be saved."] = "ВНИМАНИЕ! ShadowedUF_Indicators устарел с v4 и теперь встроен. Удалите ShadowedUF_Indicators, ваша конфигурация сохранится."
L["What group this aura belongs to, this is where you will find it when configuring."] = "Группа, к которой относится аура; там вы найдёте её при настройке."
L["What type of auras should be enlarged, use the scaled aura size option to change the size."] = "Какие ауры увеличивать; размер задаётся опцией масштаба аур."
L["When to show cooldown rings on auras"] = "Когда показывать кольца перезарядки на аурах"
L["Whether auras you casted should be shown"] = "Показывать ли ауры, применённые вами"
L["Whether to show any auras casted by the boss"] = "Показывать ли ауры, применённые боссом"
L["Whether to show any debuffs you can remove, cure or steal."] = "Показывать ли дебаффы, которые вы можете снять, излечить или украсть."
L["Whether to show auras that do not fall into the above categories."] = "Показывать ли ауры, не попадающие в категории выше."
L["Whether to show buffs that you cannot cast."] = "Показывать ли баффы, которые вы не можете применить."
L["Which auras to show in this frame"] = "Какие ауры показывать в этой рамке"
L["Which tooltips to hide while in combat. Aura tooltips are shown natively by the game and can be kept visible."] = "Какие подсказки скрывать в бою. Подсказки аур игра показывает сама, их можно оставить видимыми."
L["You can link auras together using this, for example you can link Mark of the Wild to Gift of the Wild so if the player has Mark of the Wild but not Gift of the Wild, it will still show Mark of the Wild as if they had Gift of the Wild."] = "Ауры можно связывать: например, свяжите Знак дикой природы с Даром дикой природы — если у игрока есть Знак, но нет Дара, основная аура всё равно будет показана, как будто Дар есть."
L["You can override what aura is enabled on a per-class basis, note that if the aura is disabled through the main listing, then your class settings here will not matter."] = "Можно переопределить активную ауру для каждого класса; если аура отключена в основном списке, классовые настройки не действуют."
L["You can then use these filters in the Auras tab of the player, target, raid, etc. modules, or assign them per zone type in the Zone Filters tab."] = "Затем эти фильтры можно использовать во вкладке «Ауры» модулей игрока, цели, рейда и т.д. или назначать по типу зоны во вкладке «Фильтры зон»."
L["You have entered combat, test mode has been disabled."] = "Вы вступили в бой, тестовый режим отключён."
L["You have entered combat, unit frames have been locked."] = "Вы вступили в бой, рамки юнитов заблокированы."
L["You must enter a name."] = "Нужно ввести имя."
L["You will need to create a filter in the Custom filters tab before you can assign it here."] = "Сначала создайте фильтр во вкладке «Пользовательские фильтры», чтобы назначить его здесь."
L["Your buffs flagged to show on raid frames in combat."] = "Ваши баффы, помеченные для отображения на рейдовых рамках в бою."
L["Zone Filters"] = "Фильтры зон"
...
L["Adds a bar indicating how much time is left on your statue."] = "Добавляет полосу, показывающую оставшееся время вашей статуи."
...
L["Ammo"] = "Боеприпасы"
L["Amount of health missing, if none is missing nothing is shown. Uses a short format, -18500 is shown as -18.5k, values below 10000 are formatted as is.|n|nIn combat, may show -0 at full health due to secret values."] = "Значение убывания здоровья, если здоровье полное, ничего не отображается. Используется формат сокращения, -18500 отображается как -18.5k, значения ниже 10000 не форматируются.|n|nВ бою может показывать -0 при полном здоровье из-за секретных значений."
L["Amount of power missing,  if none is missing nothing is shown. Uses a short format, -13850 is shown as 13.8k, values below 10000 are formatted as is.|n|nIn combat, may show -0 at full power due to secret values."] = "Значение убывания энергии, если энергия полная, ничего не отображается. Используется формат сокращения, -13850 отображается как 13.8k, значения ниже 10000 не форматируются.|n|nВ бою может показывать -0 при полной энергии из-за секретных значений."
L["Anchor point"] = "Точка привязки"
L["Override General text settings"] = "Переопределить общие настройки текста"
L["Use the text settings below instead of the ones from the General tab."] = "Использовать настройки текста ниже вместо настроек со вкладки «Общие»."
...
L["Controls how the anchored aura frame is positioned relative to the parent frame."] = "Определяет, как привязанная рамка аур располагается относительно родительской рамки."
L["New row"] = "Новая строка"
L["Sequential"] = "Последовательно"
...
L["Bar alignment"] = "Выравнивание полосы"
L["Bar alpha"] = "Прозрачнос полосы"
L["Bar spacing"] = "Промежуток полос"
L["Bar thickness"] = "Толщина полосы"
...
L["Direction aura icons fill within a row."] = "Направление заполнения иконок аур в ряду."
L["Direction new rows of auras stack."] = "Направление размещения новых рядов аур."
...
L["Aura Indicators"] = "Индикаторы аур"
L["Inset"] = "Вставить"
...
L["Name (NSRT Nickname)"] = "Имя (Ник NSRT)"
L["Name (NSRT Nickname/Class colored)"] = "Имя (Ник NSRT/Цвет класса)"
L["Name (NSRT Nickname/Abbreviated)"] = "Имя (Ник NSRT/Сокр.)"
L["Unit name replaced by NSRT nickname if available. Falls back to the regular name if NSRT is not installed or no nickname is set."] = "Имя юнита заменяется на ник NSRT, если доступен. Показывает обычное имя, если NSRT не установлен или ник не задан."
L["NSRT nickname colored by class. Falls back to the regular name if NSRT is not installed or no nickname is set.|n|nIn combat, shows uncolored name on enemy units."] = "Ник NSRT, окрашенный по классу. Показывает обычное имя, если NSRT не установлен или ник не задан.|n|nВ бою показывает неокрашенное имя на вражеских юнитах."
L["Abbreviated NSRT nickname (if longer than 10 characters). Falls back to the regular name if NSRT is not installed or no nickname is set.|n|nIn combat, shows full (non-abbreviated) name on enemy units."] = "Сокращённый ник NSRT (если длиннее 10 символов). Показывает обычное имя, если NSRT не установлен или ник не задан.|n|nВ бою показывает полное имя на вражеских юнитах."
...
L["Anchor mode"] = "Режим привязки"
L["Health bar"] = "Полоса здоровья"
L["Frame"] = "Рамка"
L["Health fill (reverse)"] = "Заполнение здоровья (обратное)"
L["Health bar (fill + reverse overlay)"] = "Полоса здоровья (заполнение + обратное наложение)"
L["Bar anchored to the end of the health bar, growing outward (uses overflow limit)."] = "Полоса привязана к концу полосы здоровья, растёт наружу (использует предел переполнения)."
L["Bar anchored to the frame edge, growing inward with reverse fill."] = "Полоса привязана к краю рамки, растёт внутрь с обратным заполнением."
L["Bar overlaid on the health bar with reverse fill, growing inward from the health fill edge."] = "Полоса наложена на полосу здоровья с обратным заполнением, растёт внутрь от края заполнения."
L["Bar fills missing health then overflows as reverse overlay on the health bar."] = "Полоса заполняет недостающее здоровье, затем переполняется как обратное наложение на полосу здоровья."
...
L["Percent of the players current health that's being healed, if they have 100,000 total health and 15,000 is incoming then 15% is shown.|n|nIn combat, only works on allied units."] = "Отображать количество исцеления в процентах от общего количества здоровья, если у цели 100,000 общего здоровья и получаемое исцеление равняется 15,000, тогда будет показано 15%."
...
L["Position"] = "Позиция"
L["Position of the bar when thickness is less than 100%."] = "Положение полосы, когда толщина менее 100%."
L["Power"] = "Энергия"
...
L["Relative size of the bar perpendicular to the fill direction. 100% matches the health bar exactly."] = "Относительный размер полосы перпендикулярно направлению заполнения. 100% точно соответствует полосе здоровья."
L["Resources"] = "Ресурсы"
...
L["Shorten damage absorption, if 13,000 damage will be absorbed, it will show 13k."] = "Укороченное значение поглащения урона. Если поглащается 13,000 урона - отображается 13k."
...
L["Unit name colored by class.|n|nIn combat, shows uncolored name on enemy units."] = "Окраска названия/имени игрока/существа в соответствии с цветом его класса.|n|nВ бою имя вражеских юнитов отображается без цвета класса."
...
L["When the unit is mising health, the [missinghp] tag is shown, when they are at full health then the [name] tag is shown. This lets you see -1000 when they are missing 1000 HP, but their name when they are not missing any.|n|nIn combat, health deficit only works on allied units."] = "Когда игрок/существо теряет здоровье, отображается тег [missinghp], когда значение здоровье максимальное тогда отображается тег [name]. Это позволит вам увидеть -1000 при потере 1000 очков здоровья.|n|nВ бою дефицит здоровья работает только на союзных юнитах."
...
-- 12.0 Aura filters
L["Big defensive cooldowns"] = "Мощные защитные способности"
L["Center Left"] = "Центр слева"
L["Center Right"] = "Центр справа"
L["Class-filtered auras"] = "Ауры по классу"
L["Crowd control effects"] = "Эффекты контроля"
L["Dispellable by me"] = "Рассеиваемые мной"
L["External defensives"] = "Внешние защитные"
L["Group/Raid Auras"] = "Ауры группы/рейда"
L["Important auras"] = "Важные ауры"
L["My big defensives"] = "Мои мощные защитные"
L["My class auras"] = "Мои ауры класса"
L["My external defensives"] = "Мои внешние защитные"
L["Purgeable/Stealable"] = "Очищаемые/Похищаемые"
L["Only shows buffs marked as important by Blizzard. Server-maintained list."] = "Показывает только баффы, отмеченные Blizzard как важные. Список поддерживается сервером."
L["Only shows buffs you cast yourself."] = "Показывает только баффы, наложенные вами."
L["Only shows buff types that your class can cast (e.g. a Priest sees Fortitude and Renew), regardless of who cast them."] = "Показывает только типы баффов, которые может накладывать ваш класс (напр. Жрец видит Стойкость и Обновление), независимо от того, кто их наложил."
L["Only shows crowd control effects (Stun, Root, Silence, Fear, Polymorph, Cyclone, etc.)."] = "Показывает только эффекты контроля (Оглушение, Обездвиживание, Молчание, Страх, Превращение, Смерч, и т.д.)."
L["Only shows debuffs marked as important by Blizzard. Server-maintained list."] = "Показывает только дебаффы, отмеченные Blizzard как важные. Список поддерживается сервером."
L["Only shows debuffs you applied yourself."] = "Показывает только дебаффы, наложенные вами."
L["Only shows defensive buffs received from another player (Pain Suppression, Ironbark, Life Cocoon, etc.)."] = "Показывает только защитные баффы от другого игрока (Подавление боли, Железная кора, Кокон жизни, и т.д.)."
L["Only shows external defensives that you cast on someone."] = "Показывает только внешние защитные, наложенные вами на кого-то."
L["Only shows major defensive cooldowns (Shield Wall, Ice Block, Divine Shield, etc.). List maintained by Blizzard."] = "Показывает только мощные защитные перезарядки (Стена щита, Ледяная глыба, Божественный щит, и т.д.). Список Blizzard."
L["Only shows major defensive cooldowns that you activated yourself."] = "Показывает только мощные защитные, активированные вами."
L["Only shows your class buff types that you cast yourself."] = "Показывает только типы баффов вашего класса, наложенные вами."
L["Shows all buffs on the unit without any filtering."] = "Показывает все баффы цели без фильтрации."
