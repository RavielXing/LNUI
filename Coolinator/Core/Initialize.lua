...
if CallbackRegistryMixin then
  addonTable.CallbackRegistry = CreateFromMixins(CallbackRegistryMixin)
  if addonTable.CallbackRegistry then
    if addonTable.CallbackRegistry.OnLoad then
      addonTable.CallbackRegistry:OnLoad()
    end
    if addonTable.CallbackRegistry.GenerateCallbackEvents and addonTable.Constants and addonTable.Constants.Events then
...
local function ImportExisting()
  if not addonTable.Utilities.GetSpecID or not addonTable.Core.GetExistingLayoutName
     or not addonTable.Config.Get or not addonTable.Core.GenerateCoolinatorLayoutFromExisting then
    return false
  end
...
function addonTable.Core.AutoGenerateLayout(name)
  if not addonTable.Core.GenerateDefaultCDMLayout then return end
  local spec = addonTable.Utilities.GetSpecID()
...
local function TriggerUpdate()
  if not addonTable.Core.GetFont or not addonTable.Core.AutoGenerateLayout
     or not addonTable.Core.GenerateSpellOverrides or not addonTable.Core.GetCurrentDesign
     or not addonTable.Core.ApplyPresets or not addonTable.Core.GetCDMMappingAuras
     or not addonTable.Core.StoreKeyBindings then
    return
  end
...
frame:SetScript("OnEvent", function(_, eventName, data1, data2)
  ...
  elseif eventName == "SPELLS_CHANGED" then
    if addonTable.Core.GetCurrentDesign and addonTable.Core.GetCDMOrderAurasOnly then
      local layout = addonTable.Core.GetCurrentDesign()
...
EventUtil.ContinueOnPlayerLogin(function()
  if not addonTable.Core.GetFont then return end
  addonTable.CurrentNumberFont = addonTable.Core.GetFont()
  ...
  if addonTable.Core.GetCurrentDesign then
    local layout = addonTable.Core.GetCurrentDesign()
    if layout and addonTable.Core.ApplyPresets then
      addonTable.Core.ApplyPresets(layout)
    end
  end
  if addonTable.Utilities.InitFrameWithMixin then
    if addonTable.Display and addonTable.Display.LayoutManagerNextMixin then
    ...
    if addonTable.Designer and addonTable.Designer.LayoutManagerMixin then
...
startupFrame:SetScript("OnEvent", function(_, eventName)
  ...
  seen = seen - 1
  if seen == 0 then
    if ImportExisting and ImportExisting() then
      if addonTable.Core.GetCurrentDesign and addonTable.Core.ApplyPresets then
        local layout = addonTable.Core.GetCurrentDesign()
        addonTable.Core.ApplyPresets(layout)
      end
    end
...
function addonTable.Core.GetCurrentDesign()
  if not addonTable.Utilities.GetSpecID or not addonTable.Config.Get then return end
  local spec = addonTable.Utilities.GetSpecID()
