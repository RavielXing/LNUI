...
function Addon:PrintTradeLog(ListMode, AltName, SelectedDate)
	-- 确保Output对象已初始化
	if not Output.background or not Output.title or not Output.export then
		Output:Initialize()
	end
	...
	for i = StartPoint, #TradeLog do
		if (AltName and SelectedDate and TradeLog[i].PlayerName == AltName and TradeLog[i].Date == SelectedDate) or (AltName and not SelectedDate and TradeLog[i].PlayerName == AltName) or (not AltName and SelectedDate and TradeLog[i].Date == SelectedDate) or (not AltName and not SelectedDate) then
...
function Frame:ADDON_LOADED(Name)
	...
	-- 动态加载时，若玩家已在世界中则立即初始化小地图按钮
	if UnitName("player") then
		if Addon.LDB and Addon.LDBIcon then
			if not MinimapIcon.initialized then
		...
		if Config.ShowMinimapIcon then
			...
			elseif MinimapIcon.Minimap then
	...
	-- print(string.format(L["|cFFBA55D3MailLogger - 快跑兄弟修复版|r v%s|cFFB0C4DE has been loaded.|r"], Addon.Version))
...
local function CreateMailButton()
	-- 如果按钮已存在，直接返回
	if MailLoggerMailButton then
		return
	end
	
	-- 创建按钮
	local button = CreateFrame("Button", "MailLoggerMailButton", MailFrame, "UIPanelButtonTemplate")
	
	-- 设置按钮层级为顶层
	button:SetFrameStrata("DIALOG")      -- 设为对话框层级，高于普通界面
	button:SetFrameLevel(100)            -- 提高同层级内的显示顺序
	button:SetToplevel(true)             -- 独立于父级，始终显示在最前
	
	button:SetSize(50, 18)               -- 进一步缩小按钮尺寸
	button:SetText(L["Trade Log"])
	button:SetPoint("TOPRIGHT", MailFrame, "TOPRIGHT", -50, -2) -- 微调垂直位置，与收件箱文字完全对齐
	
	-- 进一步缩小按钮文字
	local font = button:GetFontString()
	font:SetFontObject(GameFontNormalSmall)
	font:SetFont(font:GetFont(), 10, "OUTLINE")
	...
	button:SetScript("OnClick", function()
		...
		ShowUIElements(true)
		PrintAndRefresh("TRADE")
	end)
	
	-- 设置按钮提示信息
	button:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
		GameTooltip:SetText(L["Trade Log"])
		GameTooltip:AddLine(L["Click to view trade logs"])
		GameTooltip:Show()
	end)
