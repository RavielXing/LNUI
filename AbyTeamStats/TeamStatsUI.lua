...
local f = WW:Frame(TS.FRAME_NAME, UIParent, "ButtonFrameTemplate"):TOP(-110, -230):Size(728,450):Hide():SetAlpha(0.95):SetMovable(true):SetToplevel(true)
...
function TS.SetTab(index)
    ...
    local ids, names, tips, widths = tab.specialIDs or tab.ids, tab.names, tab.tips, tab.widths
    if tab.dynamic_columns then
        if tab.tab == "千钧一发" then
            ids, names, tips, widths = TS.GetDynamicColumnsForCuttingEdge()
        elseif tab.tab == "坚韧钥石" then
            ids, names, tips, widths = TS.GetDynamicColumnsForKeystone()
        end
    end
    ...
    for i, col in ipairs(TS.cols) do
        local hide = index ~= 1 and col.onlyTab1
        if col.header then
            header_id = header_id + 1
            local btn = f.headers[header_id]
            if hide or header_id > #ids + TS.NUM_FIX_HEADERS then
                btn:Hide()
            else
                if header_id > TS.NUM_FIX_HEADERS then
                    ...
                    btn.ids = ids[colIdx]
                    btn:Show()
                    btn:SetText(names[colIdx])
                    btn.tooltipTitle = names[colIdx]
                    btn.tooltipText = tips[colIdx] or
                    ...
                    col.width = widths and widths[colIdx] or TS.DEFAULT_COL_WIDTH
                else
                    btn:Show()
    ...
    minWidth = minWidth and minWidth + 35 or MIN_WIDTH
