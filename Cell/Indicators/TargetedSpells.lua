...
local function ShowCastsSecret(b, activeCasts, numCasts)
    ...
    if displayMode ~= "Border" then
        ...
        for i = 1, num do
            ...
            -- FIX: UnitIsUnit may return nil if the target unit no longer exists; convert nil to false
            local isTargeted = UnitIsUnit(cast.sourceUnit .. "target", unit)
            if isTargeted == nil then isTargeted = false end
            ts[i]:SetAlphaFromBoolean(isTargeted)
        end
        -- Hide unused slots
    ...
    if displayMode ~= "Icons" and numCasts > 0 then
        ts:ShowGlow(unpack(Cell.vars.targetedSpellsGlow))
        -- FIX: same nil protection for glow frame
        local isTargeted = UnitIsUnit(activeCasts[1].sourceUnit .. "target", unit)
        if isTargeted == nil then isTargeted = false end
        ts.tsGlowFrame:SetAlphaFromBoolean(isTargeted)
    else
        ts:HideGlow()
    end
end
