        ...
        if not value or value ~= self.states.unit then
            ...
            if self.__unitGuid then -- self.__unitGuid is deleted when hide
                ...
                if not self.isSpotlight then
                    -- 12.0+: GUID may be secret in restricted contexts; can't use as table key
                    if F.IsValueNonSecret(self.__unitGuid) then
            ...
            if self.__unitName then
                if not self.isSpotlight then
                    -- 12.0+: name may be secret in restricted contexts; guard for safety
                    if F.IsValueNonSecret(self.__unitName) then
