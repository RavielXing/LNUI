...
function BP:ScheduleListenHoldRelease()
	...
	self._listenHoldTimer = C_Timer.NewTimer(holdSec, function()
		self._listenHoldTimer = nil
...
function BP:GetSearchCooldownRemaining()
	local remaining = GF.FindGroup and GF.FindGroup:GetSearchCooldownRemaining(self._lastSearchAt) or 0
	local nativeCooldown = GF.Search and GF.Search.GetCooldownRemaining
		and GF.Search:GetCooldownRemaining() or 0
	local requestBarrier = GF.Search and GF.Search.GetRequestBarrierRemaining
		and GF.Search:GetRequestBarrierRemaining() or 0
	return math.max(remaining, nativeCooldown, requestBarrier)
end
...
function BP:ScheduleQueuedCooldownSearch()
	...
	if C_Timer and C_Timer.NewTimer then
		self._queuedCooldownSearchTimer = C_Timer.NewTimer(delay, runQueuedSearch)
	else
		runQueuedSearch()
	end
end
...
function BP:ScheduleSearchCooldownUI()
	...
	local function tick()
		...
		if C_Timer and C_Timer.NewTimer then
			BP._searchCooldownTimer = C_Timer.NewTimer(0.5, tick)
		end
	end
	if C_Timer and C_Timer.NewTimer then
...
function BP:UpdateSearchHint()
	...
	elseif not searchable then
		local mythicPlusScopeUnavailable = self.selection
			and GF.LFGWorkspaceView and GF.LFGWorkspaceView.IsMythicPlusActive
			and GF.LFGWorkspaceView:IsMythicPlusActive()
			and GF.LFGWorkspacePolicy and GF.LFGWorkspacePolicy.GetSeasonActivityIDs
			and #GF.LFGWorkspacePolicy:GetSeasonActivityIDs() == 0
		if mythicPlusScopeUnavailable then
			text = L.MPLUS_LFG_SCOPE_UNAVAILABLE
...
function BP:SetWorkspaceContext(context)
...
function BP:GetWorkspaceContext()
	return self.workspaceContext
end

function BP:GetActiveSearchContext()
...
function BP:DoSearch(opts)
	...
	self._searchFailed = false
	local workspaceContext = self.workspaceContext
		or (GF.LFGWorkspaceView and GF.LFGWorkspaceView.GetContext
			and GF.LFGWorkspaceView:GetContext())
		or {}
	local searchContext = {
	...
	self._activeSearchContext = searchContext
	...
	if GF.FindGroup then
		started = GF.FindGroup:RunSearch(self.selection, searchContext)
	end
	if not started then
		self.awaitingGFSearch = false
		self._resultToken = nil
		...
		local nativeCooldown = GF.Search and GF.Search.GetCooldownRemaining
			and GF.Search:GetCooldownRemaining() or 0
		if math.max(requestBarrier, nativeCooldown) > 0 then
			self:QueueSearchAfterCooldown(opts)
			self:ScheduleSearchCooldownUI()
			return
		end
