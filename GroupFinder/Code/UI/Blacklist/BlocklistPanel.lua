...
local HEADER_HEIGHT = math.abs(BLOCK_NAV_BOTTOM_OFFSET - BLOCK_NAV_TOP_OFFSET)
local HEADER_TEXT_SIZE = GF.BROWSE_HEADER_TEXT_SIZE or 13
...
local ROW_TEXTURE_SOURCE_WIDTH = 564
local ROW_TEXTURE_SOURCE_CAP_WIDTH = 18
local ROW_TEXTURE_DISPLAY_HEIGHT = 33
local ROW_TEXTURE_DISPLAY_CAP_WIDTH = math.max(1, math.floor(ROW_TEXTURE_DISPLAY_HEIGHT * ROW_TEXTURE_SOURCE_CAP_WIDTH / 52 + 0.5))
...
local function setHoverGradient(texture, direction, r, g, b, alpha)
	...
	if texture.SetGradient and CreateColor then
		if direction == "LEFT" then
			texture:SetGradient("HORIZONTAL", CreateColor(r, g, b, 0), CreateColor(r, g, b, alpha))
		elseif direction == "RIGHT" then
			texture:SetGradient("HORIZONTAL", CreateColor(r, g, b, alpha), CreateColor(r, g, b, 0))
		else
...
local function setRowHoverColor(row, r, g, b, alpha)
	setHoverGradient(row.HoverLeft, "LEFT", r, g, b, alpha)
	setHoverGradient(row.HoverRight, "RIGHT", r, g, b, alpha)
	if row.Hover then
...
local function setRowHover(row, shown)
	...
	if row.Hover then
		row.Hover:SetShown(row._hoverShown)
	end
	if row.HoverLeft then
		row.HoverLeft:SetShown(row._hoverShown)
	end
	if row.HoverRight then
		row.HoverRight:SetShown(row._hoverShown)
	end
end
...
local function createActionButton(parent, text)
	local button = GF.UI.CreatePanelButton(parent, text, ACTION_BUTTON_WIDTH, true)
...
local function setSaveButtonEnabled(row, enabled)
	...
	row.SaveButton:SetAlpha(enabled and 1 or 0.42)
end
...
local function createRow(parent)
	...
	row.HoverLeft:SetSize(ROW_HOVER_FADE_WIDTH, ROW_HOVER_HEIGHT)
	row.HoverLeft:SetTexture(WHITE)
	row.HoverLeft:Hide()
	row.HoverRight = row:CreateTexture(nil, "BORDER", nil, 3)
	row.HoverRight:SetPoint("RIGHT", row, "RIGHT", -ROW_HOVER_INSET_X, ROW_HOVER_OFFSET_Y)
	row.HoverRight:SetSize(ROW_HOVER_FADE_WIDTH, ROW_HOVER_HEIGHT)
	row.HoverRight:SetTexture(WHITE)
	row.HoverRight:Hide()
	row.Hover = row:CreateTexture(nil, "BORDER", nil, 3)
	row.Hover:SetPoint("LEFT", row.HoverLeft, "RIGHT", 0, 0)
	row.Hover:SetPoint("RIGHT", row.HoverRight, "LEFT", 0, 0)
	row.Hover:SetHeight(ROW_HOVER_HEIGHT)
	row.Hover:SetTexture(WHITE)
	row.Hover:Hide()
	setRowHoverColor(row, 1, 0.08, 0.05, ROW_HOVER_ALPHA)
...
local function createRemoveDialog()
	...
	dialog.YesButton = GF.UI.CreatePanelButton(dialog, T("BLOCKLIST_CONFIRM_YES", "Yes"), REMOVE_DIALOG_BUTTON_WIDTH, true)
	dialog.YesButton:SetSize(REMOVE_DIALOG_BUTTON_WIDTH, REMOVE_DIALOG_BUTTON_HEIGHT)
	dialog.NoButton = GF.UI.CreatePanelButton(dialog, T("BLOCKLIST_CONFIRM_NO", "No"), REMOVE_DIALOG_BUTTON_WIDTH, true)
...
function BP:Init(parent)
	...
	setFontSize(empty, GF.BROWSE_EMPTY_TEXT_SIZE or 14)
