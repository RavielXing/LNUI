...
function GF.Search:GetRequestBarrierRemaining()
	local expiresAt = tonumber(self.abandonedUntil)
	if not expiresAt or not GetTime then
		return 0
	end
...
function GF.Search:GetCooldownRemaining()
	if not self.lastNativeSearchAt or not GetTime then
		return 0
	end
	local remaining = (GF.SEARCH_COOLDOWN or 3) - (GetTime() - self.lastNativeSearchAt)
	return math.max(0, remaining)
end

function GF.Search:AbandonActiveRequest()
	local wasInFlight = self.inFlight == true
	if wasInFlight and GetTime then
		self.abandonedUntil = GetTime() + (GF.SEARCH_TIMEOUT_SECONDS or 8)
...
function GF.Search:ConsumeAbandonedEvent()
...
function GF.Search:GetContext()
	return self.context
end

function GF.Search:MatchesContext(context)
...
function GF.Search:Run(selection, context)
	if self.inFlight then
		self:AbandonActiveRequest()
		return false
	end
	if self:GetRequestBarrierRemaining() > 0 or self:GetCooldownRemaining() > 0 then
		return false
	end
	self:Reset()
	...
	if #scopes == 1 then
		if scopeNeedsAggregatedResults(scopes[1]) then
			...
			local started = self:_RunPendingScope()
			if not started then
				self:Reset()
			end
			return started
		end
		local started = self:_RunScope(scopes[1])
		if not started then
			self:Reset()
		end
		return started
...
local function resultMatchesActivitySet(info, activitySet)
	...
	if type(info.activityIDs) == "table" then
		for _, activityID in ipairs(info.activityIDs) do
			if activityID and activitySet[activityID] == true then
				return true
			end
		end
	end
	local activityID = GF.SearchResultSnapshot
		and GF.SearchResultSnapshot.GetPrimaryActivityID
		and GF.SearchResultSnapshot.GetPrimaryActivityID(info)
	if not activityID then
		activityID = info.activityID
	end
	return activityID and activitySet[activityID] == true or false
end
...
function GF.Search:OnSearchResults()
	...
	for _, resultID in ipairs(ids or {}) do
		...
		if resultActivitySet then
			...
			keep = resultMatchesActivitySet(info, resultActivitySet)
		end
		if keep then
