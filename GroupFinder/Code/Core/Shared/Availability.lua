...
local PREMADE_GROUP_CREATE_MIN_LEVEL = 50
...
local function getPremadeCreateLevelRequirementMessage()
	local L = GF.L or {}
	return L.PREMADE_CREATE_LEVEL_REQUIRED or "You must reach level 50 to create a premade group"
end

local function isPlayerBelowPremadeCreateLevel()
	...
	return ok
		...
		and level > 0
		and level < PREMADE_GROUP_CREATE_MIN_LEVEL
end
...
local function getPremadeUseStatus()
	if C_LFGInfo and C_LFGInfo.CanPlayerUsePremadeGroup then
		...
		if canUse == false then
			if isPlayerBelowPremadeCreateLevel() then
				return false, getPremadeCreateLevelRequirementMessage()
			end
