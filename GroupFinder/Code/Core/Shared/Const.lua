...
GF.CONTROL_ATLAS_TEXTURE = GF.ADDON_ART_UI_PATH .. "ControlAtlas.png"
GF.CONTROL_ATLAS_WIDTH = 136
GF.CONTROL_ATLAS_HEIGHT = 304
GF.CONTROL_ATLAS_REGIONS = {
	filterCheck = { 4, 4, 128, 64 },
	inputBox = { 4, 84, 108, 216 },
}
GF.FILTER_CHECK_ATLAS_TEXTURE = GF.CONTROL_ATLAS_TEXTURE
GF.FILTER_CHECK_ATLAS_REGION = GF.CONTROL_ATLAS_REGIONS.filterCheck
GF.FILTER_CHECK_ATLAS_COORDS = {
	checked = {
		(4 + 0.5) / GF.CONTROL_ATLAS_WIDTH,
		(4 + 64 - 0.5) / GF.CONTROL_ATLAS_WIDTH,
		(4 + 0.5) / GF.CONTROL_ATLAS_HEIGHT,
		(4 + 64 - 0.5) / GF.CONTROL_ATLAS_HEIGHT,
	},
	normal = {
		(4 + 64 + 0.5) / GF.CONTROL_ATLAS_WIDTH,
		(4 + 128 - 0.5) / GF.CONTROL_ATLAS_WIDTH,
		(4 + 0.5) / GF.CONTROL_ATLAS_HEIGHT,
		(4 + 64 - 0.5) / GF.CONTROL_ATLAS_HEIGHT,
	},
}
GF.FILTER_CHECK_ATLAS_COORDS.hover = GF.FILTER_CHECK_ATLAS_COORDS.checked
GF.FILTER_INPUT_ATLAS_COORDS = {
	normal = GF.FILTER_CHECK_ATLAS_COORDS.normal,
	hover = GF.FILTER_CHECK_ATLAS_COORDS.hover,
}
...
GF.INPUT_BOX_ATLAS_TEXTURE = GF.CONTROL_ATLAS_TEXTURE
GF.INPUT_BOX_ATLAS_REGION = GF.CONTROL_ATLAS_REGIONS.inputBox
GF.INPUT_BOX_ATLAS_STATE_PIXELS = {
	hover = { 4, 84, 108, 108 },
	normal = { 7, 192, 104, 108 },
}
-- Card chrome shares these effective crops. The packed normal cell contains
-- asymmetric transparent edge padding and must not be sampled as 108px wide.
GF.CONTROL_ATLAS_FRAME_STATE_PIXELS = {
	highlighted = GF.INPUT_BOX_ATLAS_STATE_PIXELS.hover,
	normal = GF.INPUT_BOX_ATLAS_STATE_PIXELS.normal,
}
GF.CONTROL_ATLAS_FRAME_SOURCE_MARGIN = 16
GF.CONTROL_ATLAS_FRAME_DISPLAY_MARGIN = 8
...
GF.MPLUS_BROWSE_THRESHOLD_OPEN_INPUT_W = 28
...
-- 正式服商城商品卡片（Blizzard_CatalogShopSharedTemplates）。
-- 保留给其他共享消费者，以及 ControlAtlas renderer 不可用时的兼容回退。
...
-- 大秘境左栏仅在共享 ControlAtlas 卡片 renderer 不可用时回退到
-- 幻化衣柜 collected 常态框。
GF.MPLUS_BROWSE_CARD_FRAME_ATLAS = "transmog-wardrobe-border-collected"
GF.MPLUS_BROWSE_CARD_BORDER_SLICE_MARGINS = {
	left = 16,
	top = 16,
	right = 16,
	bottom = 16,
}
-- 背景落在 collected atlas 的可见描边内侧：底部保留原生 12px
-- 安全边，左右和顶部比原生内容窗口各放宽 2px。
GF.MPLUS_BROWSE_CARD_BACKGROUND_INSETS = {
	left = 7,
	top = 4,
	right = 7,
	bottom = 12,
...
GF.FILTER_NUMBER_INPUT_CAP_W = 9
