...
local function setCommonSquareTexCoord(texture, state)
	if not texture then
		return
	end
	state = state or COMMON_BUTTON_STATES.normal
...
local function setupNavExpander(row)
	...
	setCommonSquareTexCoord(row.expander, COMMON_BUTTON_STATES.normal)
...
local function applyNavExpander(row)
	...
	local bgState = (row._navPressed and COMMON_BUTTON_STATES.pressed)
		or (hover and COMMON_BUTTON_STATES.highlight)
	...
	setDesaturated(row.expander, dimmed and not hover)
	if dimmed and not hover then
		row.expander:SetVertexColor(0.58, 0.58, 0.58, 1)
	else
		row.expander:SetVertexColor(1, 1, 1, disabled and 0.5 or 1)
	end
	if row.expanderGlyph then
