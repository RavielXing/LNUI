...
-- ============================================
-- 12.0 兼容修复：手动实现 EquipmentManager_UnpackLocation
-- 原函数在 FrameXML/EquipmentManager.lua 中定义，12.0 版本被移除
-- ============================================
if not EquipmentManager_UnpackLocation then
    function EquipmentManager_UnpackLocation(location)
        if not location or location <= 0 then
            return nil, nil, nil, nil, nil, nil, nil, nil
        end
        
        -- 使用 bit32 或 bit 库（WoW 12.0 主要使用 bit32）
        local bitlib = bit32 or bit
        if not bitlib then
            return nil, nil, nil, nil, nil, nil, nil, nil
        end
        
        -- 位掩码定义（基于 FrameXML/EquipmentManager.lua）
        local ITEM_INVENTORY_LOCATION_PLAYER = 0x1
        local ITEM_INVENTORY_LOCATION_BANK = 0x2
        local ITEM_INVENTORY_LOCATION_BAGS = 0x4
        local ITEM_INVENTORY_LOCATION_VOIDSTORAGE = 0x8
        
        -- 解析标志位（低4位）
        local player = bitlib.band(location, ITEM_INVENTORY_LOCATION_PLAYER) ~= 0
        local bank = bitlib.band(location, ITEM_INVENTORY_LOCATION_BANK) ~= 0
        local bags = bitlib.band(location, ITEM_INVENTORY_LOCATION_BAGS) ~= 0
        local voidStorage = bitlib.band(location, ITEM_INVENTORY_LOCATION_VOIDSTORAGE) ~= 0
        
        -- 解析 slot (bits 4-15) - 12位
        local slot = bitlib.band(bitlib.rshift(location, 4), 0xFFF)
        if slot == 0 then slot = nil end
        
        -- 解析 bag (bits 16-23) - 8位
        local bag = bitlib.band(bitlib.rshift(location, 16), 0xFF)
        if bag == 0 then bag = nil end
        
        -- 解析 void tab (bits 24-29) - 6位
        local voidTab = bitlib.band(bitlib.rshift(location, 24), 0x3F)
        if voidTab == 0 then voidTab = nil end
        
        -- 解析 void slot (bits 30-35) - 6位
        local voidSlot = bitlib.band(bitlib.rshift(location, 30), 0x3F)
        if voidSlot == 0 then voidSlot = nil end
        
        -- 返回：player, bank, bags, voidStorage, slot, bag, voidTab, voidSlot
        return player, bank, bags, voidStorage, slot, bag, voidTab, voidSlot
    end
end
-- ============================================
...
addon.name = LOCALE_zhCN and "一键换装" or "一鍵換裝"
    ...
    local function findLocationsForSlots()
        ...
        for loc,v in pairs(avail) do
            ...
            if bags and slot and bag then
                ...
                -- 12.0 修复：添加 nil 检查，防止空槽位导致 table index is nil
                if link then
                    rev[link] = loc
                end
            end
