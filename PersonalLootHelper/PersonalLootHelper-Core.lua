			...
			if ShouldShowLootedItem(lootedItem) then
				...
				if IsPlayer(lootedItem[LOOTER_NAME]) then
					...
					if lootedItemStatus == STATUS_REQUESTED or lootedItemStatus == STATUS_OFFERED or lootedItem[CONFIRMATION_MESSAGE] ~= nil then
						text = "你提出进行交易" --.. lootedItem[FULL_ITEM_INFO][FII_ITEM]
					else
						text = "你可以进行交易" --.. lootedItem[FULL_ITEM_INFO][FII_ITEM]
					end
				else
					color = COLOR_NON_PLAYER_LOOTED_ITEM
					if IsPLHUser(lootedItem[LOOTER_NAME]) then
						text = Ambiguate(lootedItem[LOOTER_NAME], 'all') .. "提出交易" --.. lootedItem[FULL_ITEM_INFO][FII_ITEM]
					else
				...
				elseif lootedItemStatus == STATUS_OFFERED then
					if IsPlayer(lootedItem[LOOTER_NAME]) then
						...
						text = "已向" .. Ambiguate(offeredName, 'all') .. "提出交易!  *** 打开交易以赠送物品 ***"
					elseif lootedItem[SELECTED_REQUESTOR_INDEX] == 1 then
						text = Ambiguate(lootedItem[LOOTER_NAME], 'all') .. "向你提供了此物品!  *** 打开交易以接收物品 ***"
					else
						text = "物品不再可用 - 它已被拾取者保留或交易"
					end
...
--					text = "You whispered " .. Ambiguate(lootedItem[LOOTER_NAME], 'all') .. " to request this item"
					...
					for sortOrder = 1, #lootedItem[REQUESTORS] do
						for requestorIndex = 1, #lootedItem[REQUESTORS] do
							...
							if requestor[REQUESTOR_SORT_ORDER] == sortOrder then
								text = Ambiguate(requestor[REQUESTOR_NAME], 'all') .. "投掷了" .. requestor[REQUESTOR_ROLL] .. "来获取" .. requestor[REQUESTOR_REQUEST_TYPE]
				...
				if IsPlayer(lootedItem[LOOTER_NAME]) then
					if lootedItem[CONFIRMATION_MESSAGE] ~= nil then
						CreateButton("好的", 50, INDENT, verticalOffset, PLH_DoClearConfirmationMessage, lootedItemIndex)
					elseif lootedItemStatus == STATUS_OFFERED then
						CreateButton("好的", 50, INDENT, verticalOffset, PLH_DoHideItem, lootedItemIndex)
					else
						CreateButton("保留", 50, INDENT, verticalOffset, PLH_DoKeepItem, lootedItemIndex)
					end

					if lootedItemStatus == STATUS_DEFAULT then
						CreateButton("向团队提供交易", 120, INDENT + 65, verticalOffset, PLH_DoTradeItem, lootedItemIndex)
					elseif lootedItemStatus == STATUS_REQUESTED then
						CreateButton("向选定的玩家提供交易", 180, INDENT + 65, verticalOffset, PLH_DoOfferItem, lootedItemIndex)
					end
				else
					if lootedItem[CONFIRMATION_MESSAGE] ~= nil then
						CreateButton("好的", 50, INDENT, verticalOffset, PLH_DoClearConfirmationMessage, lootedItemIndex)
					elseif lootedItemStatus ~= STATUS_AVAILABLE then
						...
						CreateButton("不要", 50, INDENT, verticalOffset, PLH_DoHideItem, lootedItemIndex)

						if IsPLHUser(lootedItem[LOOTER_NAME]) then
--							if IsAnUpgradeForCharacter(lootedItem[FULL_ITEM_INFO], PLH_GetFullName('player'), PLH_PREFS[PLH_PREFS_ILVL_THRESHOLD]) then
								CreateButton("主天赋", 50, INDENT + 65, verticalOffset, PLH_DoRequestItem, lootedItemIndex, REQUEST_TYPE_MAIN_SPEC)
								CreateButton("副天赋", 50, INDENT + 115, verticalOffset, PLH_DoRequestItem, lootedItemIndex, REQUEST_TYPE_OFF_SPEC)
--							end
							...
							local button = CreateButton("求装备", 80, INDENT + 65, verticalOffset)
							button:SetScript('OnClick', function(self)
								if PLH_PREFS[PLH_PREFS_WHISPER_MESSAGE] == nil or PLH_PREFS[PLH_PREFS_WHISPER_MESSAGE] == '' then
									PLH_SendUserMessage("你必须在PLH选项中配置个性化的悄悄话消息[/plh]才能悄悄话请求战利品.")
								else
							...
							button:SetScript('OnEnter', function(self)
								if IsWidgetVisible(self, 0) then
									if PLH_META[PLH_SHOW_WHISPER_WARNING] == nil or PLH_META[PLH_SHOW_WHISPER_WARNING] then
										-- local warning = "悄悄话消息将是: \"" .. PLH_GetWhisperMessage(lootedItems[lootedItemIndex][FULL_ITEM_INFO][FII_ITEM]) .. "\"\n你可以通过输入\"/plh\"来更改此消息"
...
local function CreateLootedItemsDisplay()
	...
	if PLH_META[PLH_LAST_SEEN_MESSAGE_VERSION] and tonumber(PLH_META[PLH_LAST_SEEN_MESSAGE_VERSION]) and tonumber(PLH_META[PLH_LAST_SEEN_MESSAGE_VERSION]) < 2.0 then
		local welcomeText = "欢迎使用个人拾取助手(PLH) 2.0!!\n\n" ..
			"此窗口将显示可交易的战利品!\n\n" ..
			"通过左键拖动来移动.\n" ..
			"通过右下角的图标调整大小.\n" ..
			"使用中间鼠标按钮滚动.\n\n" ..
			"当可交易的战利品掉落时窗口将自动出现.\n" ..
			"拾取者可以保留或提供物品. " ..
			"其他玩家可以选择放弃或请求主天赋(MS), 副天赋(OS), 或幻化(XMOG)\n\n" ..
			"如果拾取者未使用PLH, ni 可以对他们进行悄悄话. 请在选项界面配置悄悄话消息.\n\n" ..
			"输入\"/plh\"以查看选项界面."
...
--	PLH_SendDebugMessage('Entering PLH_ProcessOfferItemMessage (' .. looterName .. ', ' .. lootedItemID .. ', ' .. requestorName .. ')')
	...
	if lootedItem ~= nil and ShouldAnnounceTrades() then
		PLH_SendBroadcast(looterName .. '提供了' .. lootedItem[FULL_ITEM_INFO][FII_ITEM] .. '给' .. requestorName, false)
	end
		...
		if lootedItem ~= nil then
			if lootedItem[STATUS] == STATUS_AVAILABLE or lootedItem[STATUS] == STATUS_REQUESTED then
				...
				if requestorIndex == nil then
					...
					PLH_SendDebugMessage('请求已收到 ' .. requestorName .. '; 忽略新请求')
				end
			end
		end
	end
...
--	PLH_SendDebugMessage('Entering PLH_ProcessVersionMessage (' .. plhUser .. ', ' .. version .. ')')
	...
	if plhUsers[plhUser] == nil or plhUsers[plhUser] ~= version then
		...
		if version > C_AddOns.GetAddOnMetadata('PersonalLootHelper', 'Version') and not showedVersionAlert then
			PLH_SendUserMessage("你的个人拾取助手版本已过时. 你可以通过Twitch应用程序下载版本" .. version .. "或在curseforge.com上搜索PLH")
...
function PLH_DoTradeItem(lootedItemIndex)
	...
	if not PLH_PREFS[PLH_PREFS_SKIP_CONFIRMATION] then
		lootedItem[CONFIRMATION_MESSAGE] = "谢谢你! 其他PLH用户已被告知该物品可用.\n如果有人请求该物品, PLH会通知你."
	end
	UpdateLootedItemsDisplay()
...
function PLH_DoRequestItem(lootedItemIndex, requestType)
	...
	lootedItem[STATUS] = STATUS_REQUESTED
	if not PLH_PREFS[PLH_PREFS_SKIP_CONFIRMATION] then
		lootedItem[CONFIRMATION_MESSAGE] = "你的请求正在发送给" .. Ambiguate(lootedItem[LOOTER_NAME], 'all') .. ".\nPLH会在他们做出决定时通知你."
	end
	UpdateLootedItemsDisplay()
			...
			if isTradeable then
				...
				if PLH_PREFS[PLH_PREFS_SHOW_TRADEABLE_ALERT] then
					if isAnUpgradeForAnyCharacter then
						...
						PLH_SendAlert('你可以交易' .. fullItemInfo[FII_ITEM] .. ', 这对' .. names.. '来说是一个装备等级升级')
						PlaySound(600)  -- 'GLUECREATECHARACTERBUTTON'
					end
				end			
			end
...
--		end
	...
	elseif string.sub(string.upper(msg), 1, 5) == 'TRADE' or string.sub(string.upper(msg), 1, 5) == 'OFFER' then
		...
		PLH_SendUserMessage("未知参数. 可选项有:\n" ..
			"/plh  :  打开界面选项\n" ..
			"/plh show  :  显示战利品窗口\n" ..
			"/plh hide  :  隐藏战利品窗口\n" ..
--			"/plh config  :  open interface options\n" ..
			"/plh offer [item]  :  向团队中的其他PLH用户提供[item]"
		)
	end
end
