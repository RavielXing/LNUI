...
local function IsLaonongFan(name, realm)
    ...
    -- 无 realm 时模糊匹配（非队长成员的 name 不带服务器）
    local prefix = name .. "-"
    for k in pairs(p) do
        if k:find(prefix, 1, true) then return true end
    end
    return false
end
...
local function HasLaonongFan(activity)
    ...
    for i = 1, numMembers do
        ...
        if ok and member and type(member.name) == "string" and member.name ~= "" then
            local realm = member.isLeader and GetRealmFromLeaderName(info.leaderName) or nil
