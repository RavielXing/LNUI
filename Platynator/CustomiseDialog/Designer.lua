...
function addonTable.CustomiseDialog.GetMainDesigner(parent)
  ...
  do
    ...
    local dispelColor = CreateColorFromHexString("fffc982f")
    assert(dispelAsset and dispelColor)
    for kind, w in pairs(auraContainers) do
      ...
      for index, tex in ipairs(textures[kind]) do
        ...
        buff.Dispel = CreateFrame("Frame", nil, buff)
        buff.Dispel:SetAllPoints()
        buff.DispelBorder = buff.Dispel:CreateTexture(nil, "OVERLAY")
        buff.DispelBorder:SetAllPoints()
        buff.DispelBorder:SetScale(dispelAsset.scaleModifier)
        buff.DispelBorder:SetTexture(dispelAsset.file)
        buff.DispelBorder:SetTextureSliceMargins(dispelAsset.margins.left, dispelAsset.margins.top, dispelAsset.margins.right, dispelAsset.margins.bottom)
        buff.DispelBorder:SetVertexColor(dispelColor.r, dispelColor.g, dispelColor.b)
        buff:Show()
