...
local function getInitialAddonInfo()
    for i = 1, C_AddOns.GetNumAddOns() do
        ...
        title = title:gsub(LOCALE_zhCN and "%|cff880303%[爱不易%]%|r " or "%|cff880303%[愛不易%]%|r ", "")
        ...
        addonInfo[name:lower()] = {
            ...
            lod = C_AddOns.IsAddOnLoadOnDemand(i),
            vendor = C_AddOns.GetAddOnMetadata(i, "X-Vendor") == "LNui", --LNui
        ...
        --U1DisableAddOn(name); --关闭插件, 在VARIABLE_LOADED事件时再打开 --改为在 ConfigsLoaded 里面调用
    end
...
function U1RegisterAddon(name, infoReg)
    ...
    if infoReg.temporarilyForceDisable and infoReg.vendor then
        if C_AddOns.GetAddOnEnableState(name, U1PlayerGuid) == 2 then
            U1_message_temporarilyForceDisable = (U1_message_temporarilyForceDisable or 0) + 1
            U1Message(format(LOCALE_zhCN and "版本更新，暂时禁用|cFFFFFF00【%s】|r(%s)，请耐心等待修复" or "版本更新，暫時禁用|cFFFFFF00【%s】|r(%s)，請耐心等待修復", infoReg.title, infoReg.name))
        end
        U1DisableAddOn(name)
        ...
        infoReg.desc = LOCALE_zhCN and "|cffff0000暂时禁用，请耐心等待更新|r``" or "|cffff0000暫時禁用，請耐心等待更新|r``" .. (infoReg.desc or "")
    end
end
...
function U1ConfigsLoaded()
    ...
    for k,v in pairs(addonInfo) do
        ...
        if v.registered and v.defaultEnable == nil and v.parent == nil and not v.hide and k~=strlower(_) then
            U1Message(format(LOCALE_zhCN and "插件 %s 没有设置defaultEnable" or "插件 %s 沒有設置defaultEnable", v.name))
        end
    ...
    for k,v in pairs(addonInfo) do
        ...
        if (v.registered and v.load~="NORMAL" and not v.realLOD) and k~=strlower(_) then
            U1DisableAddOn(k); --v.load = "NORMAL" --用来完全关闭LATER和LOGIN
        end
...
function U1GetAddonModsAndMemory(addonName)
    ...
    if (info.dummy and U1IsAddonEnabled(addonName)) or C_AddOns.IsAddOnLoaded(addonName) then
...
function U1OutputAddonState(text, addon, duration, force)
    ...
    if duration > 0 then
        local color = (duration > 700 and "ff0000") or ( duration > 550 and "ff7f00" ) or "7fff7f"
        dtext = "，用时" .. format("|cff%s%.0f|rms", color, duration)
    end
    if duration > 550 or (DEBUG_MODE and duration > 30) or force or (initComplete and not outputOnce[addon]) then
...
function U1SimulateEvent(event, ...)
    ...
    for i=1,#eventCaptured[event] do
        ...
        if #v==2 then
            ...
            -- 添加保护检查：跳过受保护的帧
            if v and v.GetScript and v:GetScript("OnEvent") and v.IsEventRegistered and 
               (event=="PLAYER_LOGIN" or v:IsEventRegistered(event)) and 
               not (v.IsProtected and v:IsProtected()) then -- 新增的保护检查
...
function U1LoadAddOnBackend(name)
    ...
    if ii.conflicts then
        for _, other in ipairs(ii.conflicts) do
            if C_AddOns.IsAddOnLoaded(other) then
                U1DisableAddOn(name)
                return false, LOCALE_zhCN and "不能与-" or "不能與-"..U1GetAddonTitle(other)..LOCALE_zhCN and "-同时开启" or "-同時開啟"
            end
        end
    end

    local iip = ii.parent and U1GetAddonInfo(ii.parent);
    if(ii.parent and not C_AddOns.IsAddOnLoaded(ii.parent) and not iip.dummy and not loadPath[ii.parent]) then
    ...
    if(ii.deps) then
        ...
        for _, dep in ipairs(deps) do
            if not C_AddOns.IsAddOnLoaded(dep) and not loadPath[dep] then
                if C_AddOns.GetAddOnEnableState(dep, U1PlayerGuid)<2 then U1EnableAddOn(dep) end --EnableAddOn会触发右侧显示面板，而右侧有连续显示的保护
    ...
    if C_AddOns.GetAddOnEnableState(name, U1PlayerGuid)<2 then U1EnableAddOn(name) end --需要加载的时候再启用. --EnableAddOn会触发右侧显示面板，而右侧有连续显示的保护
    ...
    for subName, subInfo in U1IterateAllAddons() do
        if(subInfo.parent==name) then
            ...
            if deepToggleChildren and not subInfo.ignoreLoadAll then
                ...
                if (enabled) then
                    local r2 = U1ToggleAddon(subName, enabled, nil, deepToggleChildren, bundleSim);
                ...
                if enabled and U1IsAddonEnabled(subName) then
                    local r2 = U1ToggleAddon(subName, true, "noset", deepToggleChildren, bundleSim);
                    reloadChildren =  reloadChildren or r2;
                elseif not enabled and C_AddOns.IsAddOnLoaded(subName) and U1IsAddonEnabled(subName) then
                    local r2 = U1ToggleAddon(subName, false, "noset", deepToggleChildren, bundleSim);
                    reloadChildren =  reloadChildren or r2;
                end
            end
        end
...
local function shouldLoadAddon(name, info, when)
    ...
    local should = (not info.lod or info.loadWith and C_AddOns.IsAddOnLoaded(info.loadWith)) and not C_AddOns.IsAddOnLoaded(name) and U1IsAddonEnabled(name);
...
local function processDefaultEnable()
    ...
    -- 这样并不影响在插件里用非控制台修改插件状态（因为有hook然后设置db），只可能版本更新当天在人物界面里开关插件是无效的
    local game_version, _, _, toc_version = GetBuildInfo()
    if U1DBG.lastGameVersion ~= game_version or U1DBG.lastTocVersion ~= toc_version then
        for name,info in pairs(addonInfo) do
            if db.addons[name] then
                info.originEnabled = db.addons[name] == 1
                if not info.originEnabled then U1DisableAddOn(name) end -- ConfigsLoaded里，NORMAL的不会被关闭
            end
        end
        U1DBG.lastGameVersion = game_version
        U1DBG.lastTocVersion = toc_version
    end
            ...
            if (info.defaultEnable~=nil) then
                ...
                if info.load=="NORMAL" and info.registered then
                    ...
                    elseif enabled and not info.originEnabled then
                        U1EnableAddOn(name) --forceDisables will be handled in processSpecialDeps
                    end
                end
            else
...
local function processSpecialDeps()
    ...
    for addon, deps in pairs(SpecialDependencies) do
        if select(5, C_AddOns.GetAddOnInfo(addon)) ~= "MISSING" then
            local mustDisable = true
            for _, dep in ipairs(deps) do
                if C_AddOns.GetAddOnEnableState(dep, UnitGUID("player")) ~= 0 then
                    mustDisable = false
                    break
                end
            end
            if mustDisable then
                --print("must disable", addon, C_AddOns.GetAddOnEnableState(addon, UnitName("player")) ~= 0)
                forceDisables[addon:lower()] = true
                if C_AddOns.GetAddOnEnableState(addon, UnitGUID("player")) ~= 0 then
                    U1DisableAddOn(addon)
                end
            end
        end
    end
...
function U1:ADDON_LOADED(event, name)
    if name == _ then
        ...
        if U1.restoreFromDisableAll then
            --恢复之前的状态
            U1Message(LOCALE_zhCN and "检测到只开启了控制台，还原全关之前的状态..." or "檢測到只開啟了控製臺，還原全關之前的狀態...")
            for k, state in pairs(U1DB and U1DB.addons or {}) do
                if state == 1 and k~=string.lower(_) then
                    local v = addonInfo[k]
                    if v then
                        if (v.registered and v.load~="NORMAL" and not v.realLOD) and k~=strlower(_) then
                            --U1DisableAddOn() --same as ConfigLoaded() but already disabled
                        else
                            U1EnableAddOn(k)
                        end
        ...
        local info = U1GetAddonInfo(name)
        if info then
            if info.loadConfirm then U1Message(format(LOCALE_zhCN and "插件|cffff7f7f%s(%s)|r%s" or "插件|cffff7f7f%s(%s)|r%s", info.title, info.name, info.loadConfirm)) end
...
function U1:VARIABLES_LOADED(calledFromLogin)
    ...
    for _, v in ipairs(addonsToEnable) do U1EnableAddOn(v) end
    wipe(addonsToEnable);
    ...
    if #addonToLoad > 0 then
        ...
        if(not C_AddOns.IsAddOnLoaded(name)) then
            ...
            if curr - (lastMillis or 0) > LOAD_INFO_INTERVAL then
                lastMillis = curr
                U1Message(format(LOCALE_zhCN and "延迟加载 - 正在载入插件, 剩余 |cff00ff00%d|r, 每秒 |cff00ff00%d|r" or "延遲加載 - 正在載入插件, 剩余 |cff00ff00%d|r, 每秒 |cff00ff00%d|r", #addonToLoad, loadSpeed));
            end
            ...
            if info.bundleSim then
                ...
                loaded, reason, duration = U1ToggleAddon(name, true, "noset", false, false) --later的还是不能后模拟事件，总之Toggle和Login的能模拟就好了
            else
        ...
        if #addonToLoad==0 and not initComplete then
            RunNextFrame(function()
                U1Message(L["全部插件加载完毕。"] .. (U1DBG.lastReloadTime and format(LOCALE_zhCN and "本次重载用时%.1f秒。" or "本次重载用时%.1f秒。", GetTime() - U1DBG.lastReloadTime) or "") .. U1.get_timestamp())
            end)
            ...
            C_AddOns.SaveAddOns() --非常重要，如果这里不保存一下，ESC-插件 然后点击取消会调用ResetAddOns，就会重新开启所有插件，大概是因为加载时DisableAddOn不更新存储状态
...
-- CoreIOF_OTC = InterfaceOptionsFrame_OpenToCategory --TODO:abyui10,lnui解决11.0配置选项点击无效的问题
function CoreIOF_OTC(arg)
    Settings.OpenToCategory(arg)
end

local function U1FindCategoryMatchingCategoryID(categoryName, category)
	...
	if category.subcategories then
		for index, subcategory in ipairs(category.subcategories) do
			local foundCategory = U1FindCategoryMatchingCategoryID(categoryName, subcategory)
...
function U1GetSettingCategoryByName(categoryName)
	local categoryList = SettingsPanel:GetCategoryList()
	for _, tbl in ipairs(categoryList.groups) do
		for _, category in ipairs(tbl.categories) do
			local foundCategory = securecallfunction(U1FindCategoryMatchingCategoryID, categoryName, category)
			if foundCategory then
				return foundCategory
			end
		end
...
function U1GetSettingCategoryIDByName(categoryName)
	local category = U1GetSettingCategoryByName(categoryName)
	if category then
		local id = category:GetID()
		-- 11.0+ 兼容：某些插件把 category.ID 设为字符串（如 Platynator）
		-- 返回 category 对象本身，配合下方 Hook 绕过 C API 数字限制
...
-- 修复：当 Settings.OpenToCategory 传入的是 category 对象时（字符串 ID 的情况），
-- 直接通过 SettingsPanel 打开，避免 C_SettingsUtil.OpenSettingsPanel 因收到字符串而报错
local origSettingsOpenToCategory = Settings.OpenToCategory
Settings.OpenToCategory = function(categoryID)
	if type(categoryID) == "table" and categoryID.GetID then
		SettingsPanel:Open()
		SettingsPanel:SelectCategory(categoryID)
		return
	end
	origSettingsOpenToCategory(categoryID)
end
