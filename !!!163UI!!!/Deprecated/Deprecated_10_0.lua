...
if false then --46702 暴雪修复了
    CoreOnEvent("VARIABLES_LOADED", function()
        ...
        local btn = WW:Button(name, def:GetParent(), "UIPanelButtonTemplate")
                      ...
                      :SetText('\124TInterface\\AddOns\\!!!163UI!!!\\Textures\\UI2-logo:' .. (20) .. '\124t' .. (working and "  重載保證安全  " or "  顯示插件熱鍵  "))
                      :SetScript("OnDoubleClick", function()
                    if not working then U1DBG.show_3rd_bindings = true end
                    ReloadUI()
                end)  :un()

        btn.tooltipText = "|cFFFFFF00臨時顯示插件定義的熱鍵|r\n\n因為暴雪的BUG,中文客戶端無法顯示插件自定義的熱鍵（參見https://bbs.nga.cn/read.php?tid=34037994 oyg123大佬曾發現並解決過9.1打開地圖卡頓的問題）。\n\n根據這個帖子，增加這個按鈕，'|cFFFFFF00雙擊|r'此按鈕會重載界面並顯示這些熱鍵，但是這樣可能導致界面失效的問題。所以設置完熱鍵後，請再次重載界面。"
        ...
        SettingsPanel:HookScript("OnHide", function()
            if working then
                U1Message("你之前開啟了顯示插件熱鍵功能，當前環境可能不安全，建議/rl重載界面")
            end
        end)
    end)
end
