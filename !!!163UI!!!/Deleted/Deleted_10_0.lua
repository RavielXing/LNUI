        ...
        if (quality <= 3 and equipSlot) then
            if CanIMogIt and CanIMogIt:GetTooltipText(nil, bag, slot) == CanIMogIt.UNKNOWN then
                ...
                if (StaticPopup1:IsVisible() and StaticPopup1.which == "EQUIP_BIND") then
                    ...
                    U1Message(LOCALE_zhCN and "快捷解锁外观：" or "快捷解鎖外觀：" .. link)
                    ef.time = GetTime()
                    ef.bag, ef.slot = bag, slot
                    ef:RegisterEvent("BAG_UPDATE_DELAYED")
                end
            else
                U1Message(CanIMogIt and "此物品外觀已解鎖或不能被當前角色解鎖" or "快捷解鎖失敗：未啟用幻化提示(CanIMogIt)插件")
            end
        end
    end
end)
...
do
    ...
    local DomiSetColor = { "a335ee", "0070ff", "ff0000" } --紫藍紅
    local DomiSetNameLong = { "森羅萬象(頭)", "寒冬之風(肩)", "鮮血連接(胸)" }
    local DomiSetNameShort = { "森羅", "寒冬", "鮮血" }
    local DomiShardName = { "邪惡", "冰霜", "鮮血" }
