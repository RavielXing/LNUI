...
function U1MMB_HasConflictAddons()
    return MBB_Version or MBBFrame or MBFversion or (NxMapDock and NxMapDock:IsVisible()) or C_AddOns.IsAddOnLoaded("Mappy")
...
local hasGameMenuFrameHeight = false
---被CreateUI调用的创建函数
function U1MMB_CreateContainer()
	...
	-- 11.0后游戏主菜单动态生成，高度也是动态。除非打开一次菜单，否则高度默认1（见下链接）
	-- https://github.com/Gethe/wow-ui-source/blob/7547929cf58cc69525bfe193a5444e7acb2646df/Interface/AddOns/Blizzard_SharedXML/Mainline/SharedUIPanelTemplates.xml#L3108
	-- 这里使用笨办法：尝试模拟打开主菜单让他生成高度
	if not hasGameMenuFrameHeight then
		GameMenuFrame:InitButtons()
		GameMenuFrame:Layout()
		hasGameMenuFrameHeight = true
	end
    --游戏菜单附着区域
...
function U1_MMBCreateCoordsButton()
    ...
    btn:SetMovable(true)
    btn:SetSize(68, 20)
    btn:SetPoint("TOPRIGHT",Minimap,"BOTTOMRIGHT",-67,-5)
    ...
    local function MinimapCoordsButton_OnUpdate()
        ...
        if(px == 0 and py == 0) then
            fot:SetText(LOCALE_zhCN and "无坐标" or "无坐标");
        else
            fot:SetFormattedText("%.1f-%.1f", px * 100, py * 100);
        end
    end
    CoreUIEnableTooltip(btn, LOCALE_zhCN and "当前坐标" or "當前坐標", LOCALE_zhCN and "双击左键发送到聊天框\n\n坐标框下数字为位面ID, 点击右键可隐藏。变灰时表示不确定，鼠标随便划过一个NPC或怪可确定位面ID" or "雙擊左鍵發送到聊天框\n\n坐標框下數字為位面ID, 點擊右鍵可隱藏。變灰時表示不確定，鼠標隨便劃過一個NPC或怪可確定位面ID")
