...
_G["BINDING_NAME_CLICK GameMenuButtonEditMode:LeftButton"] = LOCALE_zhCN and "编辑模式" or "Enter Edit Mode"
...
function UUI.Raise(raise)
    ...
    if(U1ChangeLogFrame and U1ChangeLogFrame:IsVisible()) then U1ChangeLogFrame:Hide() end
    local main = UUI();
    if(raise~=false)then
...
function UUI.ClickAddonCheckBox(self, name, enable, subgroup)
    ...
    if info and self:GetChecked() then
        ...
        --if #other_loaded > 0 then U1EnableAddOn(name) return ReloadUI() end
    end
    ...
    if(reasonInfo) then
        ...
        if(depNum > 0) then
            for i=1, depNum do
                ...
                local depEnabled = C_AddOns.GetAddOnEnableState(name, U1PlayerGuid)>=2
...
function UUI.Top.Create(main)
    ...
    :Button():Key("dropbutton"):Size(20,UUI.PANEL_BUTTON_HEIGHT+6):LEFT("$parent", "RIGHT", 1, -1)
    ...
    :un()
    CoreUIEnableTooltip(main.collect, LOCALE_zhCN and "额外设置" or "額外設置", LOCALE_zhCN and "一些暴雪取消了设置界面，但却有意义的设置选项" or "一些暴雪取消了設置界面，但卻有意義的設置選項")
    UUI.AddChangeWithColsButton(main.collect, LOCALE_zhCN and "额外设置" or "額外設置", LOCALE_zhCN and "额外" or "額外")
    ...
    main.changelog = TplPanelButton(main,nil, UUI.PANEL_BUTTON_HEIGHT):Set3Fonts(UUI.FONT_PANEL_BUTTON) --更新说明功能功能
    :SetScript("OnMouseUp", function(self)
        if U1ChangeLogFrame then
            CoreUIShowOrHide(U1ChangeLogFrame, not U1ChangeLogFrame:IsVisible())
            U1ChangeLogFrame:SetFrameLevel(UUI():GetFrameLevel()+10)
        end
    end)
    CoreUIEnableTooltip(main.changelog, L["更新记录"], L["点击查看老农整合包更新内容。"])
    UUI.AddChangeWithColsButton(main.changelog, L["更新记录"], L["更新"])

    local lnuiReleaseLinkDialog = "163UIUI_Release_Link_Dialog" --可以随便定义，发布地址功能
    StaticPopupDialogs[lnuiReleaseLinkDialog] = {
        text = "|cffff0000老农|r|cff00ccff整合包|r-发布地址\n按 |cff00ff00Ctrl+C|r 复制", --可以随便定义
        button1 = DONE,
        hasEditBox = 1,
        OnShow = function(self)
            local editBox = self:GetEditBox()
            editBox:SetText("https://nga.178.com/read.php?tid=3632086") --可以随便定义
            editBox:HighlightText()
            editBox:SetScript("OnKeyDown", function(_, key)
            if key == "C" and IsControlKeyDown() then
                C_Timer.After(.1, function()
                    editBox:GetParent():Hide()
                    UIErrorsFrame:AddMessage("|cff00ff00已复制，请在浏览器里打开|r")
                end)
            end
        end)
    end,
    ...
    main.lnui = TplPanelButton(main,nil, UUI.PANEL_BUTTON_HEIGHT):Set3Fonts(UUI.FONT_PANEL_BUTTON)
    :SetScript("OnMouseUp", function(self)
        StaticPopup_Show(lnuiReleaseLinkDialog)
    end)
    CoreUIEnableTooltip(main.lnui, "发布地址", "点击查看老农整合包发布地址。")
    UUI.AddChangeWithColsButton(main.lnui, "发布地址", "发布")

    -- local lnuiReleaseLinkDialog = "163UIUI_Release_Link_zhinan" --可以随便定义，发布地址功能
    -- StaticPopupDialogs[lnuiReleaseLinkDialog] = {
        -- text = "|cff19CCF9插件指南视频由  抖音主播：鲨鱼辣椒（魔兽世界） 倾情制作|r（按 |cff00ff00Ctrl+C|r 复制链接）", --可以随便定义
        -- button1 = DONE,
        -- hasEditBox = 1,
        -- OnShow = function(self)
            -- local editBox = self:GetEditBox()
            -- editBox:SetText("https://space.bilibili.com/559926/") --可以随便定义
            -- editBox:HighlightText()
            -- editBox:SetScript("OnKeyDown", function(_, key)
            -- if key == "C" and IsControlKeyDown() then
                -- C_Timer.After(.1, function()
    ...
    -- end,
        ...
        -- hideOnEscape = 1,
    -- }
    -- main.zhinan = TplPanelButton(main,nil, UUI.PANEL_BUTTON_HEIGHT):Set3Fonts(UUI.FONT_PANEL_BUTTON)
    -- :SetScript("OnMouseUp", function(self)
        -- StaticPopup_Show(lnuiReleaseLinkDialog)
    -- end)
    -- CoreUIEnableTooltip(main.zhinan, "插件指南", "点击查看老农整合包插件指南视频。")
    -- UUI.AddChangeWithColsButton(main.zhinan, "插件指南", "指南")

    CoreUIAnchor(main,"TOPRIGHT","TOPRIGHT",-28-20,-12,"RIGHT", "LEFT",-8,0, main.setting, main.collect, main.profile, main.reload, main.changelog, main.lnui);--lnui，, 指南参数main.zhinan
end
...
function UUI.Center.Create(main)
    ...
    StaticPopupDialogs["163UIUI_LOAD_ALL_CONFIRM"] = {preferredIndex = 3,
        text = LOCALE_zhCN and "您确认吗？不建议您加载全部插件\n\n老农整合了非常多的优秀插件，但玩家一般用不到全部功能。建议您在默认方案的基础上选择几个自己需要的，不然会显得有点杂乱。" or "您確認嗎？不建議您加載全部插件\n\n老農整合了非常多的優秀插件，但玩家一般用不到全部功能。建議您在默認方案的基礎上選擇幾個自己需要的，不然會顯得有點雜亂。", --LN
        button1 = YES,
        button2 = CANCEL,
    ...
    StaticPopupDialogs["163UIUI_CONFLICT_CONFIRM"] = {preferredIndex = 3,
        text = LOCALE_zhCN and "此插件与以下插件冲突：\n\n%1$s\n\n确认关闭这些插件并重载界面？" or "此插件與以下插件沖突：\n\n%1$s\n\n確認關閉這些插件並重載界面？",
        button1 = YES,
        button2 = CANCEL,
        OnAccept = function(self, data)
    ...
    StaticPopupDialogs["163UIUI_WEAK_CONFLICT_CONFIRM"] = {preferredIndex = 3,
        text = LOCALE_zhCN and "此插件与以下在功能上有重迭：\n\n%1$s\n\n同时开启它们可能导致问题，建议只开一个。你确定要同时加载本插件吗？" or "此插件與以下在功能上有重疊：\n\n%1$s\n\n同時開啟它們可能導致問題，建議只開一個。你確定要同時加載本插件嗎？",
        button1 = YES,
        button2 = CANCEL,
        OnAccept = function(self, data)
          local info = U1GetAddonInfo(data[2])
...
function UUI.Center.ScrollCreateOnButton(lineButton)
    ...
    --如果直接贴texture则高光效果不同
    local icon = b:Frame():Key("icon"):Size(UUI.ICON_W):TL(4, -6)--lnui，图标位置
...
function UUI.Right.Create(main)
    ...
    :CreateTexture(nil,"BACKGROUND"):Key("wm"):SetTexture(U1.CN and UUI.Tex'UI2-watermark' or nil):SetAlpha(0.125):Size(1, 1):BL(5, 0):up() --LN
    :un();
    --WW(scroll):CreateTexture():SetTexture(1,1,1):ALL()
...
更新面板
---------------------------------------------------------------]]
function UUI.CreateChangeLog()
    local U1ChangeLogFrame = U1ChangeLogFrame;
    if U1ChangeLogFrame == nil then
        U1ChangeLogFrame = WW:Frame('U1ChangeLogFrame', U1Frame, "BasicFrameTemplateWithInset"):Size(702, 460):TOP(60, -40):SetToplevel(1):SetFrameStrata("DIALOG"):Hide()
        U1ChangeLogFrame:SetScript("OnShow", function(U1ChangeLogFrame) RunOnNextFrame(U1ChangeLogFrame.Raise, U1ChangeLogFrame) end)
        local scroll = WW:ScrollFrame(tostring(U1ChangeLogFrame).."Scroll", U1ChangeLogFrame, "MinimalScrollFrameTemplate"):Key("scroll"):TL(12,-30):BR(-30,8)
        :un();
        scroll.scrollBarHideable = nil
        _G[scroll.ScrollBar:GetName().."Track"]:SetAlpha(0.3)
        scroll.scrollBar = scroll.ScrollBar --for CoreUIScrollSavePos
        ...
        function Text:SetText(text)
            _SetText(Text, text);
            U1ChangeLogFrameDisplay:SetHeight(Text:GetHeight())
        end
        U1ChangeLogFrame.ContentText = U1ChangeLogFrameDisplay.Text;
...
function UUI.CreateUI()
    ...
    local main = WW:Frame(U1_FRAME_NAME, UIParent, ABY_BD_TPL):TR(-650, -250):Size(800,500)  --全局位置和大小，LNui
    :Hide():SetToplevel(1)
    ...
    CoreUIEnableTooltip(search, L["搜索插件及选项"], function(self, tip)
        ...
        tip:AddLine(" ");
        tip:AddLine(L["仅老农整合包的插件才能用拼音搜索名称。"], 0,0.82,0,true);
    end)
    ...
    --游戏菜单图标
    WW:Button(nil, GameMenuFrame):Key("btn163"):TL("$parent", "TR", -35, 45):Size(90, 90)  --lnui，，logo位置和光圈大小
    ...
    :CreateTexture():SetTexture(UUI.Tex"UI2-logo"):Size(110):TL(-10, 33):up()  --lnui，，esc的logo大小
    :CreateTexture():TL(-15-4,55+7):BR(25-4, -25+7):SetTexture("Interface\\UnitPowerBarAlt\\Atramedes_Circular_Flash"):SetAlpha(0.5):ToTexture("Highlight"):up() --lnui，，esc的logo光晕位置
    :un()
    CoreUIEnableTooltip(GameMenuFrame.btn163, L["爱不易"], L["点击爱不易标志开启插件控制中心\n \nCtrl点击小地图图标可以收集/还原"])

    CoreUIEnableTooltip(AddonListDisableAllButton, LOCALE_zhCN and "提示" or "提示", LOCALE_zhCN and "点全部禁用插件后，重新加载UI可彻底禁用所有插件，可用于确认是插件问题还是暴雪界面问题。\n\n然后|cffff0000只|r选中'!!!控制台和基本库'|cffff0000这一个|r插件(一般排第一个)，再重新加载UI，可恢复全部禁用之前的状态。" or "點全部禁用插件後，重新加載UI可徹底禁用所有插件，可用於確認是插件問題還是暴雪界面問題。\n\n然後|cffff0000只|r選中'!!!控製臺和基本庫'|cffff0000這一個|r插件(一般排第一個)，再重新加載UI，可恢復全部禁用之前的狀態。") --LN
...
function U1_CreateMinimapButton()
    ...
    local ldb = LibStub("LibDataBroker-1.1"):NewDataObject(U1Name, {
        ...
        OnTooltipShow = function(tip)
            ...
            tip:AddLine(L["    爱不易（AbyUI）是新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，爱不易实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"], nil, nil, nil, 1)
            tip:AddLine(" ")
    ...
    LibDBIcon10_U1MMB.icon:SetSize(28, 28)  --LNui，小地图图标大小
    LibDBIcon10_U1MMB.icon:SetPoint("TOPLEFT", 2, 2)  --LNui，小地图图标位置
