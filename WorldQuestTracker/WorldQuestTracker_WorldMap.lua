...
worldFramePOIs.mouseoverHighlight.OnShowAnimation = detailsFramework:CreateAnimationHub(worldFramePOIs.mouseoverHighlight)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.OnShowAnimation, "scale", 1, 0.035, .7, .7, 1.1, 1.1)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.OnShowAnimation, "alpha", 1, 0.035, 0.75, .91)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.OnShowAnimation, "scale", 2, 0.035, 1.1, 1.1, 1, 1)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.OnShowAnimation, "alpha", 2, 0.035, .91, 1)

worldFramePOIs.mouseoverHighlight.PulseAnimation = detailsFramework:CreateAnimationHub(worldFramePOIs.mouseoverHighlight)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.PulseAnimation, "scale", 1, 1, 1, 1, 1.6, 1.6)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.PulseAnimation, "alpha", 1, 1, 0.75, .91)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.PulseAnimation, "scale", 2, 1, 1, 1, .6, .6)
detailsFramework:CreateAnimation(worldFramePOIs.mouseoverHighlight.PulseAnimation, "alpha", 2, 1, 1, .845)
...
local do_highlight_on_quest = function(widget, scale, color)
	...
	local r, g, b, a = detailsFramework:ParseColors(color or "white")
...
--load world quests for the current continent shown
local preLoadQuestData = function()
...
function WorldQuestTracker.CreateWorldMapWidget(mapName, index, parent)
	local newWidget = WorldQuestTracker.CreateWorldMapSquareButton(mapName, index, parent)
	return newWidget
end
...
local lazyCreateWorldWidget = function(tickerObject)
	...
	local summarySquare = WorldQuestTracker.CreateWorldMapSquareButton("WorldQuestTrackerWorldSummarySquare", i, WorldQuestTracker.WorldSummary)
...
function WorldQuestTracker.InitializeWorldWidgets()
	...
	if (not WorldQuestTracker.GetBlizzardProvider()) then
	...
	local i = 1
	local button = WorldQuestTracker.CreateWorldMapSquareButton("WorldQuestTrackerWorldSummarySquare", i, WorldQuestTracker.WorldSummary)
...
function WorldQuestTracker.ShowWorldQuestPinsOnNextFrame() --entry point
...
WorldQuestTracker.POIPins = {} --these are zone widgets placed in the world map, made with "CreateZoneWidget"
--hide all poi pins
...
function WorldQuestTracker.UpdateWorldQuestsOnWorldMap(noCache, showFade, isQuestFlaggedRecheck, forceCriteriaAnimation, questList)
	...
	if (UnitLevel("player") < 50) then --this has to be improved
		...
		if (WorldQuestTracker.db.profile and not WorldQuestTracker.db.profile.low_level_tutorial) then
			...
			-- WorldQuestTracker:Msg("World quests aren't shown because you're below level 50.") --> localize-me，lnui
		end
		return
	end
	...
	--start build the quest list to show in the world map
	for mapId, configTable in pairs(WorldQuestTracker.mapTables) do
		--get the map name
		local mapInfo = C_Map.GetMapInfo(mapId)
		local mapName = mapInfo and mapInfo.name or "wrong map id"
	...
	--at this point, all quests to show are in the table 'questsAvailable'
	...
	--the second loop finished, quests ready to show in the world map are in 'questData_AddToWorldMap'
	...
	WorldQuestTracker.UpdateAllWorldMapAnchors(worldMapID) --todo, it is using map pins
...
local updateWorldMapWidgetsScaleAndPosition = function(pinScale)
...
hooksecurefunc(WorldMapFrame.ScrollContainer, "ZoomIn", function()
	...
	local pinScale = detailsFramework:MapRangeClamped(rangeValues[1], rangeValues[2], rangeValues[3], rangeValues[4], mapScale)

	if (WorldQuestTracker.IsWorldQuestHub(WorldMapFrame.mapID)) then
		updateWorldMapWidgetsScaleAndPosition(pinScale)
	end
end)
		...
		for _, summarySquare in pairs(WorldQuestTracker.WorldSummaryQuestsSquares) do
			if (summarySquare.questID == questID and summarySquare:IsShown()) then
				...
				if (WorldQuestTracker.CheckQuestRewardDataForWidget(summarySquare)) then
					WorldQuestTracker.UpdateSquareWidget(summarySquare, summarySquare.questData)
				end
				break
			end
		end
		...
		for _, widget in pairs(WorldQuestTracker.WorldMapSmallWidgets) do
			if (widget.questID == questID and widget:IsShown()) then
				...
				if (WorldQuestTracker.CheckQuestRewardDataForWidget(widget)) then
					WorldQuestTracker.SetupWorldQuestButton(widget, true, true)
				end
				break
			end
		end
...
function WorldQuestTracker.GetWorldMapSmallWidget(questID)
...
local getTranslatedWorldMapCoordinates = function(x, y, sourceMapID, targetMapID)
	if (not x or not y) then
		return nil, nil
	end

	local newX, newY = HereBeDragons:TranslateZoneCoordinates(x, y, sourceMapID, targetMapID, false)
	if (newX and newY) then
		return newX, newY
	end

	--fallback for disconnected/inset maps where HBD cannot translate directly
...
local scheduledIconUpdate = function(questData) --~update ~iconupate
	...
	if detailsFramework.IsAddonApocalypseWow() then
		...
		if (button.AnchorFrame) then
			button.AnchorFrame:EnableMouse(false)
		end
	end

	--get a pin in the world map from the data provider
	--local pin = WorldQuestTracker.GetPin()
	--pin.Child = button
	--pin.questID = questID
	...
	WorldQuestTracker.SetupWorldQuestButton(button, questData, true)

	local newX, newY = getTranslatedWorldMapCoordinates(x, y, mapID, WorldMapFrame.mapID)
...
local lazyUpdateFunc = function(self, deltaTime) --~lazyupdate
	if (WorldMapFrame:IsShown() and #lazyUpdate.WidgetsToUpdate > 0 and WorldQuestTracker.IsWorldQuestHub(WorldMapFrame.mapID)) then
		...
		for i = 1, amountToUpdate do
			...
			if (questData) then
				local button = scheduledIconUpdate(questData)
				if button then
		...
		for questCounter, button in pairs(WorldQuestTracker.WorldMapSmallWidgets) do
			...
			if (not pin or pin.Child ~= button or not pin:IsShown()) then
				--button:Hide() --because it is not using the pin, so pin is always nil
			end
		end
...
function WorldQuestTracker.LazyUpdateWorldMapSmallIcons(addToWorldMap, questsToUpdate) --~startlazy ~update
	...
	---@type wqt_questdata[]
	lazyUpdate.WidgetsToUpdate = detailsFramework.table.copy({}, addToWorldMap)
