...
function WorldQuestTrackerAddon.OpenOptionsPanel()
    ...
    optionsFrame.TitleBar.Text:SetText("世界任务增强-设置中心")  --lnui
	...
	local tabContainer = DF:CreateTabContainer(optionsFrame, "世界任务增强 设置", "WQTOptionsPanelContainer",--lnui
	{
    ...
    --buttons moved from the statusbar
			...
			WorldQuestTracker.SetupStatusbarButton(statisticsButton, "统计")  --lnui
			if (GameCooltip.InjectQuickTooltip) then
				GameCooltip:InjectQuickTooltip(statisticsButton, "单击以显示其他角色上可用的世界任务、时间线和任务的奖励统计数据。")  --lnui
			end
			...
			local BuildFilterMenu = function()
				...
				GameCooltip:AddLine("全选")  --lnui
				GameCooltip:AddMenu(1, toggle_filters_all_on)

				GameCooltip:AddLine("全不选")  --lnui
				...
				if (WorldQuestTracker.db.profile.filter_force_show_brokenshore) then
					GameCooltip:AddLine("忽略新区域", "", 1, "orange")--lnui
					GameCooltip:AddLine("新区域的世界杯将始终展示。\n\n当前新区：\n-纳沙塔尔\n-麦卡贡。", "", 2)
					GameCooltip:AddIcon([[Interface\ICONS\70_inscription_vantus_rune_tomb]], 1, 1, 23*.54, 37*.40, 0, 1, 0, 1)
					GameCooltip:AddIcon([[Interface\BUTTONS\UI-CheckBox-Check]], 1, 2, 16, 16, 0, 1, 0, 1, overlayColor, nil, true)
				else
					GameCooltip:AddLine("忽略新区域", "", 1, "silver")
					GameCooltip:AddLine("新区域的世界杯将始终展示。\n\n当前新区：\n-纳沙塔尔\n-麦卡贡。", "", 2)
