...
local setIconForQuest = function(pin, customIcon, textureObject)
	textureObject = textureObject or pin.Display.WQTIcon
	...
	if pin and customIcon.glow and pin.Display and pin.Display.WQTGlow then
...
function WorldQuestTracker.UpdateZoneWidgets(forceUpdate)
	...
	--~locked ~poi ~areapoi
	if (not specialPinsMapBans[mapId] and false) then
				...
				if (isWorldQuest) then
					if (HaveQuestData(questID)) then
						...
						if (isWorldQuest and isNotBanned and bIsOnSameMap) then
							--local isSuppressed = WorldQuestTracker.GetBlizzardProvider():IsQuestSuppressed(questID)
							--local passFilters = WorldQuestTracker.GetBlizzardProvider():DoesWorldQuestInfoPassFilters(info)
...
function WorldQuestTracker.SetupZoneSummaryButton(summaryWidget, zoneWidget)
	...
	local okay, gold, resource, apower = WorldQuestTracker.UpdateSquareWidget(widget, widget.questData, isUsingTracker, true)
	...
	summaryWidget.timeLeftText:SetText((timeLeft > 1440 and floor(timeLeft/1440) .. "天") or (timeLeft > 60 and floor(timeLeft/60) .. "小时") or (timeLeft .. "分"))  --lnui
...
function WorldQuestTracker.UpdateZoneSummaryToggleButton(canShow)
	if (not WorldQuestTracker.ZoneSummaryToogleButton) then
		...
		DF:SetFontSize(button.Text, 10)
		DF:SetFontColor(button.Text, "orange")
		button.Text:SetText(LOCALE_zhCN and "世界任务列表" or "世界任務列表")--lnui
...
function WorldQuestTracker.CanShowZoneSummaryFrame()
	...
	if (canShow) then
		if (WorldMapFrame.isMaximized) then
			...
			ZoneSumaryFrame:SetPoint("topleft", worldFramePOIs, "topleft", 2, -140) --lnui
		end
