    ...
    if (abbreviate) then
        ...
        if (numberIsSecretValue) or (self:IsSecretValue(number)) then
            ...
            if (absRealNumber >= BILLION_NUMBER) then
                abbreviatedFormat = LOCALE_zhCN and "0.1f亿" or "0.1f億";
                abbreviatedRealNumber = realNumber / BILLION_NUMBER;
            elseif (absRealNumber >= 10000) then
                abbreviatedFormat = LOCALE_zhCN and "%.0f万" or "%.0f萬";
                abbreviatedRealNumber = realNumber / 10000;
            else
...
local TT_DefaultConfig = {
	...
	showRealm = "show",
	showTarget = "last",
	targetYouText = "|CFFFF0000<<你>>", --LNui
	showGuild = true,
	showGuildRealm = "show",	
	showGuildRank = true,
	...
	-- backdrop
	enableBackdrop = true,
	tipBackdropBG = "Interface\\DialogFrame\\UI-DialogBox-Background", --LNui
	...
	classification_worldboss = "%s|r (|cFFFF0000首领|r)",--lnui
	classification_rare = "%s|r (|cFFFF33FF稀有|r)",--lnui
	classification_rareelite = "+%s|r (|cFFFF33FF稀有精英|r)",--lnui
	...
	barTexture = "Interface\\Addons\\TipTac\\media\\123",--lnui
	barHeight = 8,--lnui
	...
	-- icons
	enableIcons = true,
	iconUnitIsSecretValue = false,--lnui
	iconRaid = true,
	iconFaction = false,
	iconCombat = false,
	iconClass = false,
	...
	anchorWorldUnitType = "mouse", --LNui
	anchorWorldUnitPoint = "TOPLEFT", --LNui
	anchorWorldTipType = "mouse", --LNui
	anchorWorldTipPoint = "TOPLEFT", --LNui
	anchorFrameUnitType = "mouse", --LNui
	anchorFrameUnitPoint = "TOPLEFT", --LNui
	anchorFrameTipType = "mouse", --LNui
	anchorFrameTipPoint = "TOPLEFT", --LNui
	...
	anchorOverrideCFPoint = "BOTTOMRIGHT",
   	...
   	mouseOffsetX = 30, --LNui
   	mouseOffsetY = 0, --LNui
    top = 115, --没有top和left就会显示锚点框体
    left = (GetScreenWidth() or 2000) - -550,
...
function tt:ToggleOptions()
	...
	if (loaded) then
		...
		tt:AddMessageToChatFrame("{caption:" .. MOD_NAME .. "}: {错误：无法加载 " .. MOD_NAME .. " 设置: [{highlight:" .. _G["ADDON_" .. reason] .. "}]。 请在角色选择界面启用插件。}"); -- see UIParentLoadAddOn()
	end
end
...
function tt:SetAddonCompartmentText(tip)
	...
	tip:AddLine(TT_COLOR.text.default:WrapTextInColorCode("点击打开Tiptac设置"));
end
...
LibFroznFunctions:RegisterNewSlashCommands(MOD_NAME, { "/tip", "/tiptac" }, function(msg)
	...
	if (parameters[1] == "reset") then
		wipe(cfg);
		tt:ApplyConfig();
		tt:AddMessageToChatFrame("{caption:" .. MOD_NAME .. "}: 所有设置{highlight:" .. MOD_NAME .. "} 已重置为默认值。");
		return;
	end
	
	-- invalid command
	local versionWoW, build = GetBuildInfo();
	local versionTipTac = C_AddOns.GetAddOnMetadata(MOD_NAME, "版本");
	...
	tt:AddMessageToChatFrame("以下{highlight:parameters}对此插件有效：");
	tt:AddMessageToChatFrame("  {highlight:anchor} = 显示提示工具的锚点");
	tt:AddMessageToChatFrame("  {highlight:reset} = 重置全部设置为默认值");
end);
...
function tt:SetTipBackdropConfig()
	...
	if (currentConfig.tipBackdropBG == "nil") then
		...
		if (TT_IsConfigLoaded) and (not LibFroznFunctions:TextureExists(currentConfig.tipBackdropBG)) then
			...
			self:AddMessageToChatFrame("{caption:" .. MOD_NAME .. "}: {错误:选项标签{highlight:背景}中未设置有效的背景材质，已切换为默认背景材质。}");
		end
...
function tt:SetFontToGameTooltip()
	...
	if (cfg.modifyFonts) then
		...
		if (not LibFroznFunctions:FontExists(cfg.fontFace)) then
			...
			self:AddMessageToChatFrame("{caption:" .. MOD_NAME .. "}: {错误：在选项标签{highlight:Font}未找到有效字体设置，切換回默认字体。}");			
		end
...
LibFroznFunctions:RegisterForGroupEvents(MOD_NAME, {
	...
	OnTipRescaled = function(self, TT_CacheForFrames, tip, currentDisplayParams)
		...
		-- 装备对比节流修复：避免频繁刷新导致卡顿
		local now = GetTime()
		if now - (tip._ttLastShoppingRefresh or 0) > 0.05 then
			tip._ttLastShoppingRefresh = now
...
function tt:SetAnchorToTip(tip)
	...
	-- WoW 12.0 fix: throttle to avoid FPS drops when comparison tooltips are shown.
	-- The old code called this every frame via OnUpdate, causing severe stuttering.
	local now = GetTime()
	if now - (tip._ttLastShoppingRefresh or 0) > 0.05 then
		tip._ttLastShoppingRefresh = now
		LibFroznFunctions:RefreshAnchorShoppingTooltips(tip);
