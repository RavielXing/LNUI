...
-- Available functions:
	...
	-- ns.pluginHandler:Refresh()
		-- Invoked by HandyNotes. See notes below about throttling and why it's necessary.
	...
	-- ns.handyNotesPinIterator( t, prev )
		...
		-- ns.PassAchievementChecks( pin ) is invoked as necessary. It's in the Functions_Achievements file
		-- ns.PassQuestChecks( pin ) is invoked as necessary. It's in the Functions_Quests file
		-- ns.PassPetChecks( pin ) is invoked as necessary. It's in the Functions_Pets file
	...
	-- ns.pluginHandler:OnEnter( mapFile, coord )
		...
		-- Invokes ns.AchievementTooltipLines in the Functions_Achievements file as necessary
		-- Invokes ns.PetTooltipLines in the Functions_Pets file as necessary
		-- Invokes ns.QuestTooltipLines in the Functions_Quests file as necessary
		-- Invokes ns.AddOnSpecificTooltipLines in the Core file as necessary
	...
	-- AddOn compartment is supported
...
-- Supported fields in the Common module: pin.guide, pin.tip, pin.title, pin.name, and series .title
-- Supported fields in the Options moduels: All field names and field tooltips
-- Supported fields in Functions_XXXX modules: All .name
...
function ns.PassGeneralChecks( pin )

	-- Must be kept lean and mean as it is invoked everywhere and often for each pin and pin element.
...
local function NewClusterPins()
	...
	if ( ns.clusterNames == nil ) or ( #ns.clusterNames == 0 ) then return end
...
local function SetupPinClusterCoordinates()
	...
	-- Thus ( #ns.clusterNames or #ns.clusterCoords ) minus 1 is the number of pins surrounding the centre pin.
	...
	-- Following code was tested in Retail and MoP Classic too, end of October 2025.
...
-- Note the optional Interface option setup functions below. Found in the Options_Achievements or Options_xxx files if at all.
...
local function SetupSavedPlayerOptions()
	...
	-- Reminder: There might be other AddOns using different Engine versions and with/without the Achievements package.
...
local function OnUpdate()
	if GetTime() > ( ns.saveClusterTime or 0 ) then
		NewClusterPins()
		ns.saveClusterTime = GetTime() + fastrandom( 60, 180 )
	end
...
-- The ns.PassAdditionalAchievementChecks() function is in the Common_Achievements file, if used.
...
function ns.CompletionShow( completed, whatever, name, completionS )
	-- name/completionS are optional. It allows for "whatever" to be completed account wide (all green text) but with the
	-- character name red to indicate that that character hasn't completed it. An example is the Classic game differentiating
	-- between per character and per account achievement progress.
	
	if completed == nil then
		if whatever then GameTooltip:AddLine( ns.L[ whatever ] ) end
	else
		GameTooltip:AddDoubleLine( ( whatever and ns.L[ whatever ] or " " ), ( ( completed == true ) and
...
-- The second passed parameter (colour) is optional and will override ns.colour.plaintext / ns.colour.guide
...
local function CorrectMapPhase( old, mapID )
	...
	-- Next look for a pin.noZidormi and also bypass here if not nil.
	...
	-- if old=nil (ie. pin.zidormiOldPhase= nil) then returned values will be nil or true (LUA binaries are actually trinaries).
							
	-- Most zones have two possible buffs to test. Blizzard are allocating a different buff spell ID depending upon how the player
	-- "entered" the phase. It matters if a player flew out then back into a zone, relogged, fresh login, and so on.
	
	-- Tirisfal has a quirk whereby the player might need to do Zidormi TWICE to go to the "old times" but this routine still works.

	-- The one problem is Darkshore. Sometimes the zidormi buff is not applied. This can result in the AddOn falsely flagging that
	-- the player is in a new phase. A hack is to check for Alliance/Horde control status.
	...
	-- Effectively ensuring that we're looking at the Minimap or that the World Map is for the current zone.
	...
	if (  mapID == 17 ) or (  mapID == 18 ) or (  mapID == 2070 ) or -- Blasted Lands, 2 x Tirisfal Glades
		...
		for i = 1, 40 do -- 40 is rather arbitrary these days I think
			...
			if auraData.spellId then -- Look for Time Travelling buff
				if ( auraData.spellId == 176111 ) or ( auraData.spellId == 372329 ) or -- Blasted Lands
						...
						( auraData.spellId == 255152 ) or ( auraData.spellId == 264606 ) or -- Silithus
						( auraData.spellId == 317785 ) then -- Uldum
					return old
				elseif (  mapID == 62 ) then
					if ( auraData.spellId == 281117 ) or ( auraData.spellId == 281115 ) then -- Horde or Alliance controlled
						-- This is a "new times" phase
						return not old
					end
				end
			end
...
function ns.pluginHandler:OnEnter( mapFile, coord )
	...
	if ( pin.cluster == nil ) and ( pin.series or ns.useAsDefaultSeries ) and
				ns.series[ ( pin.series or ns.useAsDefaultSeries ) ].title then
		if ns.series[ ( pin.series or ns.useAsDefaultSeries ) ].colour then
			GameTooltip:SetText( ns.StringSubstitutions( ns.series[ ( pin.series or ns.useAsDefaultSeries ) ].colour )
						..ns.series[ ( pin.series or ns.useAsDefaultSeries ) ].title  )
		else
	...
	if ns.AchievementTooltipLines then ns.AchievementTooltipLines( pin, mapFile ) end
	if ns.PetTooltipLines then ns.PetTooltipLines( pin, mapFile ) end
	...
	if ( _G[ ns.db ].ShowCoordinates == true ) and ( pin.noCoords == nil ) then
		GameTooltip:AddLine( ( ns.colour.highlight ) .."(" ..format( "%.02f", ( floor( coord / 10000 ) / 100 ) )
