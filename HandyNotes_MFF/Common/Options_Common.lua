...
function ns.InterfaceOptions()
	...
	do
		local name = ns.colour.highlight ..ns.StringSubstitutions( ns.L[ "Map Pin Size" ] )
...
function ns.InterfaceOptionsAccess()
	local commandStr = ns.L[ "ChatCommands" ]
	for i, c in ipairs( ns.slashCommands ) do -- Defined in Data_XXXX
		commandStr = commandStr ..ns.colour.highlight ..c ..ns.colour.plaintext
		if i < #ns.slashCommands then
			commandStr = commandStr ..", "
		end
	end
	ns.optionsLayout:AddInitializer( CreateSettingsListSectionHeaderInitializer( ns.colour.plaintext ..ns.L[ "TryMinimapMenu" ]
		.."\n" ..commandStr ) )
end
