...
local frame = UICreateInterfaceOptionPage("MerchantExOptionFrame", "商人助手  "..addon.version, nil, nil, UIParent)
...
group:AddButton(L["guild repair"], "guild")
group:AddButton(L["sell"], "sell")
group:AddButton(LOCALE_zhCN and "保留9.1新宠物要的物品" or "保留9.1新寵物要的物品", "keep910")
...
local function OnHyperlinkCapture(item)
	...
	if list.category == "exception" and quality ~= 0 and quality ~= 1 and quality ~= 3 then
    	UIErrorsFrame:AddMessage(L["must be gray, white, or blue items"], 1, 0, 0)
    	return
	end
...
local function ChatFrame_OnHyperlinkShow_hook(link)
	local editbox
...
if ChatFrameMixin then
    for i = 1, (Constants.ChatFrameConstants.MaxChatWindows or 10) do
        ...
        if chatFrame and chatFrame.HookScript then
            chatFrame:HookScript("OnHyperlinkClick", function(self, link) ChatFrame_OnHyperlinkShow_hook(link) end)
        end
    end
else
    hooksecurefunc("ChatFrame_OnHyperlinkShow", function(self, link) ChatFrame_OnHyperlinkShow_hook(link) end)
end
...
frame:SetScript("OnEvent", function(self, event, arg)
	if event == "VARIABLES_LOADED" then
        ...
        do
            ...
            btn:SetPoint('TOPLEFT', 55, 0)  --lnui
            btn:SetFrameLevel(MerchantFrame:GetFrameLevel()+9999)--LNui			
            btn:SetSize(80,23)
            btn:SetText'商人助手'
