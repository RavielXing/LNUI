...
local function BuildWishlist()
    ...
    local prefix = "^" .. myKey .. "/"
    for key, spec in pairs((GearInsight.BisData or {}).specs or {}) do
        if type(key) == "string" and type(spec) == "table" and key:find(prefix) then
        ...
        if t.isTier and GearInsight.BuildFillerList and bd then
            ...
            if armor then
                ...
                if ok and list and list[1] then
                    ...
                    t.itemId   = f.itemId
                    t.name     = f.nameCn or f.itemName or t.name
                    t.bonusIDs = t.bonusIDs or f.bonusIDs
...
ev:SetScript("OnEvent", function(_, event, msg, playerName)
    ...
    local link = msg:match("(|c%x+|Hitem:.-|h%[.-%]|h|r)") or ("item:" .. itemId)
