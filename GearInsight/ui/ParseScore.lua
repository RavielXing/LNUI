...
local function tier(p)
    if p >= 100 then return "ffe5cc80", "传说"
    elseif p >= 99 then return "ffe268a8", "粉"
    elseif p >= 95 then return "ffff8000", "橙"
    elseif p >= 75 then return "ffa335ee", "紫"
    elseif p >= 50 then return "ff0070ff", "蓝"
    elseif p >= 25 then return "ff1eff00", "绿"
    else return "ff9d9d9d", "灰" end
end
...
local function showCard(encName, amount, p, src)
    ...
    if not card then
        ...
        card.share:SetSize(70, 20); card.share:SetPoint("BOTTOMRIGHT", -8, 8); card.share:SetText("分享")
    ...
    card.line1:SetText(("|cFFFFD100[%s]|r  你 ≈ |c%sp%d %s|r"):format(encName or "?", hex, math.floor(p + 0.5), _tn))
    local np, gap = gapToNext(amount, p, GearInsight._lastCurve or {})
    if np and gap then
        card.line2:SetText(("DPS %s · 距 p%d 还差 ~%s"):format(BreakUpLargeNumbers(math.floor(amount)), np, BreakUpLargeNumbers(math.floor(gap))))
    else
        card.line2:SetText(("DPS %s · 数据源 %s"):format(BreakUpLargeNumbers(math.floor(amount)), src or "?"))
    end
    card.share:SetScript("OnClick", function()
        local txt = ("[GearInsight] %s 我打出 p%d(%s)! DPS %d ——你的呢？")
            :format(encName or "", math.floor(p + 0.5), _tn, math.floor(amount))
        if GearInsight.ShowCopyText then GearInsight:ShowCopyText(txt, "Ctrl+C 复制分享", "Parse 分享") end
    end)
...
local function evaluate(encID, encName, difficultyID)
    ...
    if not amount then
        GearInsight:Print("|cffff8000[Parse]|r 没读到你的伤害(确认 Details! 开着)。"); return
    end
...
SlashCmdList["GIPARSE"] = function()
    local key = curveKey()
    local cd = key and GearInsightParseCurve and GearInsightParseCurve[key]
    if not cd or not cd.raid then GearInsight:Print("当前专精暂无 parse 曲线(熊D 已有；其余待全量)。"); return end
