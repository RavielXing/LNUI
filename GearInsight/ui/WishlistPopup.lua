...
function GearInsight:WishPopup(who, link, why, itemId, force)
    ...
    if itemId and itemId > 0 and not tostring(link):find("%[") and GearInsight.ItemName then
        local want = itemId
        GearInsight.ItemName(want, function()
            if f._itemId == want and f:IsShown() then
                local lnk = select(2, GetItemInfo(want))
                if lnk then f.item:SetText(lnk) end
            end
        end)
    end
    ...
    -- ⛔⛔ 剩余秒数存在**帧上**（f._left）而不是闭包局部变量：悬停物品链接时要暂停，
    --    松开还得接着走。存在闭包里就只能 Cancel 不能续 —— 那样鼠标扫过一次窗口
    --    就永远不会自动关了。
    if f._timer then f._timer:Cancel(); f._timer = nil end
    f._left = SHOW_SEC
    f.cd:SetText(("|cff888888%ds|r"):format(f._left))
    f:StartTick()
end
