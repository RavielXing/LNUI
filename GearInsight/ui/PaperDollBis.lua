...
-- ⛔ 这里原来有一份 mergePool 抄本，已删：合池请用 GearInsight.MergePairPool。
--   同一份数据各建各的列表正是 2026-09-07「属性页和主面板推荐不同件」的根因。
...
local function enchName(e)
    ...
    if GearInsight.EnchName then
        local n = GearInsight.EnchName(e, "")
...
-- 身上这件的**真实装等**（要读链接，物品 id 读不出升级轨道进度）。
-- 共享的成对槽位决策要用它判「已穿的这件够不够毕业装等」。
...
local function pickForSlot(slotId, bySlot, wConf, recBySlot, data)
    ...
    if pairOther then
        ...
        -- ⛔⛔ 这里原来抄了一份：合池和「副槽取 #2」都抄到了，但主面板后来按玩家反馈
        --   陆续加的三条规则一条都没跟上 —— 戒指禁止降级推荐、饰品尽量凑一主动一被动、
        --   已穿要达到该件装等才算毕业。结果属性页给的是池子原始 #1/#2（跟 Top5 天然
        --   一致），主面板给的是另一件，玩家看到两处打架（2026-09-07 QQ 群术士报）。
