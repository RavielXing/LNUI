...
function GearInsight:BuildMplusMetaContent(parent, padX, padTop)
    ...
    if t1 then
        addLine(T("MM_TANKS", "坦克  ") .. ("%s%s%s|r %.1f%%"):format(classMark(t1), classColor(t1), rowName(t1), t1.pct or 0)
            .. (t2 and ("   %s%s%s|r %.1f%%"):format(classMark(t2), classColor(t2), rowName(t2), t2.pct or 0) or ""))
    end
    if h1 then
        addLine(T("MM_HEALS", "治疗  ") .. ("%s%s%s|r %.1f%%"):format(classMark(h1), classColor(h1), rowName(h1), h1.pct or 0)
            .. (h2 and ("   %s%s%s|r %.1f%%"):format(classMark(h2), classColor(h2), rowName(h2), h2.pct or 0) or ""))
    end
    ...
    for _, r in ipairs(M.pushUnder or {}) do
        addLine(("%s%s%s|r  -%.0f%% · %.1f%%%s")
            :format(classMark(r), classColor(r), rowName(r), r.gap or 0, r.pct or 0,
                    T("MM_IN_USE", " 在用")))
    end
