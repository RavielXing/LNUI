...
function Upgrade:GetCurrentTrack()
    local context = DB:Get("ui.selectedTab");

    if (context == "dungeons") then
        local track = DB:Get("filters.dungeon.track");
        local rank = DB:Get("filters.dungeon.rank");

        return KeystoneLoot.UpgradeTracks.dungeon[track][rank];
    elseif (context == "raids") then
        local difficulty = DB:Get("filters.raid.difficulty");
        local rank = DB:Get("filters.raid.rank");

        return KeystoneLoot.UpgradeTracks.raid[difficulty][rank];
    end
end

function Upgrade:BuildItemLink(itemId)
    -- Check if upgradeable
    if (not self:IsUpgradeable(itemId)) then
    ...
    -- Get target upgrade
    local upgrade = self:GetCurrentTrack();
    if (not upgrade) then
        return string.format("item:%d", itemId);
    end
