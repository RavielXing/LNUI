...
addon:RegisterEventCallback("OnListUpdate", function()
	...
	if newName and newName == lastUnreadDisplayName then
		return
	end
	...
	if newName then
		if newName ~= lastUnreadDisplayName then
			lastUnreadDisplayName = newName
			ticker.hidden = true
			ticker:RegisterTick(0.6)
		end
	else
		lastUnreadDisplayName = nil
