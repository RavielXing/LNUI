...
function GIG:Reject(name, type)
	...
	if self.db.displayMessageAfterRejecting and declinedPlayers[name] ~= true then
		...
		if self.db.allowWhisperedTarget then
			message = message
				.. " "
				.. L["You can whisper the player once to add him/her to the whitelist temporarily."]
		end
		F.Print(message)
...
function GIG:OnInitialize()
	...
	CORE:RegisterBlackList("GroupInviteGuard", {
		priority = -1000,
		func = function(data)
	...
	self:RegisterEvent("GROUP_INVITE_CONFIRMATION", function()
