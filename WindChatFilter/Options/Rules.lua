...
local function mapOptions(context, order)
	...
	local addOptions, customOptions = customArea(context, context.rule.map.mapIDs, {
...
local function messageOptions(context, order)
			...
			help = {
				...
				args = {
					regex = {
						order = 1,
						type = "description",
						name = L["Regex is supported, however CJK support is not guaranteed because the Lua version embedded in WoW is 5.1."],
						width = "full",
					},
				},
			},
	...
	local addOptions, customOptions = customArea(context, context.rule.message.keywords, {
...
local function channelOptions(context, order)
	...
	local addOptions, customOptions = customArea(context, context.rule.channel.channelNames, {
		previewWithString = nil,
...
local function playerInfoOptions(context, order)
	...
	local playerNameAddOptions, playerNameCustomOptions = customArea(context, context.rule.playerInfo.name, {
		previewWithString = nil,
	...
	local playerRealmAddOptions, playerRealmCustomOptions = customArea(context, context.rule.playerInfo.realm, {
		previewWithString = nil,
				...
				args = {
					title = {
						...
						name = format(
							...
							format(
								L["When %s starts filtering, it will pass the contextual information to all enabled sub-modules, and the rule will only be matched if the data passes all sub-modules."],
								W.AddonName
							),
...
function F.CreateNewRule(tbl, optTbl)
	StaticPopup_Show("WIND_CHAT_FILTER_NEW_RULE", nil, nil, {
		table = tbl,
		optionTable = optTbl,
	})
end
