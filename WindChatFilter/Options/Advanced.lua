			...
			args = {
				stopInInstance = {
					...
					desc = L["Stop filtering in instance."]
						.. "\n"
						.. L["Enable this option will save resource usage, but some message may not be filtered when you change zone."],
					get = function(info)
		...
		cache = {
			...
			args = {
				doNotUseGUIDCache = {
					...
					desc = L["Disable GUID cache to reduce memory usage, but it will cause the performance drop when you are in a crowed server."],
					get = function(info)
			...
			args = {
				logLevel = {
					...
					desc = L["Only display log message that the level is higher than you choose."]
						.. "\n|cffff3860"
						.. L["Set to 1 if you do not understand the meaning of log level."]
						.. "|r",
					get = function(info)
