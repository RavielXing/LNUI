...
cacheCleaner:RegisterEvent('FRIENDLIST_UPDATE')
cacheCleaner:RegisterEvent('BN_FRIEND_LIST_SIZE_CHANGED')
cacheCleaner:RegisterEvent('GUILD_ROSTER_UPDATE')
cacheCleaner:SetScript('OnEvent', function() friendNameCache = nil end)

-- 检查队长是否是我好友（安全版本，所有操作用 pcall 包裹）
function IsFriendOfLeader(leaderFullName)
    if not leaderFullName or leaderFullName == '' then return false end
    local name = leaderFullName:match('^([^-]+)')
    if not name then return false end
    name = Ambiguate(name, 'none')
    local ok = pcall(ensureFriendCache)
    if not ok then return false end
    return friendNameCache[name] == true
end
