...
-- 内存优化：限制缓存活动对象数量，防止12.1中无限增长
local MAX_CACHED_ACTIVITIES = 120
local activityAccessOrder = {}  -- LRU 访问顺序
...
function Activity:Get(id)
    local activity = self._Objects[id]
    if activity then
        -- LRU: 更新访问顺序
        for i, v in ipairs(activityAccessOrder) do
            if v == id then
                table.remove(activityAccessOrder, i)
                break
            end
        end
        table.insert(activityAccessOrder, id)
        return activity
    end

    -- 内存优化：如果缓存已满，淘汰最久未访问的对象
    if #activityAccessOrder >= MAX_CACHED_ACTIVITIES then
        local oldId = table.remove(activityAccessOrder, 1)
        local oldActivity = self._Objects[oldId]
        if oldActivity then
            oldActivity:Release()
            self._Objects[oldId] = nil
        end
    end
...
-- 内存优化：释放方法，清理所有内部引用
function Activity:Release()
    wipe(self.killedBosses)
    self.killedBosses = nil
    self._baseSortValue = nil
    self._statusSortValue = nil
...
-- 内存优化：静态方法，清理所有缓存
function Activity:ClearAllCache()
    for id, activity in pairs(Activity._Objects) do
        if activity and activity.Release then
            activity:Release()
        end
    end
    wipe(Activity._Objects)
    wipe(activityAccessOrder)
