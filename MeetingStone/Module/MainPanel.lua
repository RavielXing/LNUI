...
local function GetLaonongFanNames(activity)
    ...
    if not C_LFGList.GetSearchResultPlayerInfo then return nil end
...
function MainPanel:OpenActivityTooltip(activity, tooltip)
    ...
    if activity:IsAnyFriend() and activity:GetNumMembers() ~= 0 then
        ...
        if leader and leaderShort and not friendText:find(leaderShort, 1, true)
            and IsFriendOfLeader(leader) then
            friendText = friendText ~= '' and (friendText .. ', ' .. leaderShort) or leaderShort
        end
        tooltip:AddLine(friendText, 1, 1, 1, true)
    end
